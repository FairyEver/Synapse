...
struct TerminalShortcutPanel: View {
    ...
    private var panel: some View {
        ...
        // 栏要有明确的显示方式，哪怕它一个字都不画。
        //
        // 这一页**没有标题**：栏上那一位被分段器占着。可 `.automatic` 是按「它有没有标题」
        // 推断的 —— 一张弹窗的根视图没有标题时，栏按**大标题**排，于是分段器下面永远留着
        // 一条给标题的带子，标题却一个字都不画。真机截图量到：分段器胶囊底到列表卡片顶
        // 57.3pt，其中 16.3pt 就是这条带子（同一张弹窗里换成内联标题的列表，这段是 41pt）。
        // 从外面看，就是分段器与列表之间空出一大块，切三段都在。（`InboxView` 记过同一个
        // 形状的坑：那里是头顶安全区把大标题顶掉了，留下同样一条空带。）
