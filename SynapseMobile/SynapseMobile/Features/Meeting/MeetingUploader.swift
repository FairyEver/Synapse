...
final class MeetingUploader {
    ...
    ///
    /// **最后一片也要比，哪怕它是唯一的一片。** 录音期间磁盘上写的是分片 m4a（开头一段
    /// 初始化 + 每 2 秒一个 `moof`），收尾时 `AVAssetWriter.finishWriting()` 会把它重排成
    /// 普通的 `ftyp + mdat + moov`——所以「录的时候看到的字节」和「定稿的字节」不是同一
    /// 份。整条录音短于一片（64 kbps 下约两分钟）时全部字节都在最后一片里，不比它就会上传
    /// 一段读不出来的音频：大小、时长、波形都对，谁也打不开。
    ///
    /// 重排**只改第 1 片和最后一片**，中间的分片字节原样不动（实测 5 片里只有 1 和 5 不同），
    /// 所以这一步的代价与录音长度无关——最长的会议也只多传两片。
    ///
