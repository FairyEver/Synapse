...
nonisolated enum MeetingAudio {
    ...
    /// 采集回调一次收多少帧。
    ///
    /// 必须**比 `peakMs` 的一个槽位短**：电平是每个槽位把这段时间里收到的采样合起来算一个
    /// RMS，回调比槽位还长的话就会有槽位一个采样都没攒到、电平掉回 0。1024 帧在 48 kHz 下
    /// 约 21 毫秒，留出余量。
    static let tapFrames = 1_024
    /// 录音页只回看最近 5 秒。
    ...
    ///
    /// **只属于录音页**：锁屏和灵动岛上那条曾按同一组数画过一段时间的圆点，2026-09-20
    /// 连波形一起删掉了（见 `RecordingLiveActivityWidget`）。实时活动现在不画任何波形，
    /// 所以这里没有第二处要跟它对齐。
