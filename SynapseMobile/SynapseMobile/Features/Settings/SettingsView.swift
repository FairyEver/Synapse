...
struct SettingsView: View {
    ...
    init(category: SettingsCategory? = nil, onSelectDesktop: @escaping () -> Void) {
    ...
    var body: some View {
        ...
        List {
            if category == nil || category == .account {
                Section("账号") {
            ...
            if category == nil || category == .terminal {
                Section {
            ...
            if category == nil || category == .desktops {
                Section("已连接的电脑") {
            ...
            // 刚表达过的那个意思。
            //
            // 诊断是当前设置分类内的子页；不参与跨功能深链选择。
            if category == nil || category == .diagnostics {
                Section {
        ...
        .noticeOverlay(model)
        .navigationTitle(category?.title ?? "我的")
