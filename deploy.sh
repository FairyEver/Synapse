...
run_cutover_steps() {
  ...
  step 14 "最终备份远程数据库" \
    backup_remote_database "$FINAL_BACKUP_FILE" || return 1
  step 15 "恢复验证最终数据库备份" \
    verify_final_backup_restore || return 1
  step 16 "备份本地 Drive 数据" \
    backup_remote_drive_fallback || return 1
  step 17 "应用生产数据库迁移" \
    deploy_remote_migrations || return 1
  step 18 "启动新服务" \
    start_new_remote_server || return 1
}
