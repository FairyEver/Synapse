...
当前应继续的动作：S06-B13“连接到会话和富交互”已完成实现和验证；先确认工作区状态，再创建独立 commit。提交后进入 S06-B14“文件引用和多 agent relay”。

继续规则：

- S06-B14 写代码前必须读取对应 CC Connect 原源码和测试；如果触碰 UI，先读 `.claude/rules/design.md` 和 `.claude/rules/ui-rules.md`。
- 每批完成后运行本批验证和通用验证三件套。
