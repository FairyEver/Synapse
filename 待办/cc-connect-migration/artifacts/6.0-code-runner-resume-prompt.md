...
当前应继续的动作：S06-B10“语音、附件和 QR onboarding”已完成实现、验证和 artifacts 回写；先提交 `stage06(S06-B10): 语音附件和 QR onboarding [CC-026 CC-027 CC-037]`，提交后确认工作区状态，再进入 S06-B11。

继续规则：

- S06-B11 写代码前必须读取对应 CC Connect 原源码和测试。
- 每批完成后运行本批验证和通用验证三件套。
