...
当前应继续的动作：S06-B09“Webhook 和 Bridge”已完成实现、验证和 artifacts 回写；先提交 `stage06(S06-B09): Webhook 和 Bridge [CC-019 CC-020]`，提交后确认工作区状态，再进入 S06-B10。

继续规则：

- S06-B10 写代码前必须读取对应 CC Connect 原源码和测试。
- 每批完成后运行本批验证和通用验证三件套。
