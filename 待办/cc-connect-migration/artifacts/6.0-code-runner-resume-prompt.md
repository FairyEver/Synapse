...
当前应继续的动作：S06-B16“Heartbeat 和 Hooks”实现和验证已完成，先确认工作区状态，再创建 S06-B16 独立 commit；提交成功后进入 S06-B17。

继续规则：

- S06-B16 已读取 `core/heartbeat.go`、`core/hooks.go`、`config/config.go`、`core/heartbeat_test.go`、`core/hooks_test.go`。
- S06-B17 写代码前必须读取 S06-B17 plan 指定的 CC Connect 原源码和测试。
- 每批完成后运行本批验证和通用验证三件套。
