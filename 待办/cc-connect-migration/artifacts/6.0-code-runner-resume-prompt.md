...
当前应继续的动作：S06-B05“Provider 和 secretRef 模型”已验证通过，先确认是否已提交；如果未提交，提交 `stage06(S06-B05): Provider 和 secretRef 模型 [CC-014]`，然后进入 S06-B06。

继续规则：

- 若继续 S06-B06，写代码前读取 S06-B06 的 CC Connect 原源码和测试。
- 每批完成后运行本批验证和通用验证三件套。
