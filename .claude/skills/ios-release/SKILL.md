---
name: ios-release
description: SynapseMobile 的 iOS 发版与装机。打包上传 TestFlight、查构建处理状态与内部群组挂载、设备安装、版本号与构建号规则、APNs 推送环境与验证。Use when 发 TestFlight、发版、打 iOS 包、mobile:release、构建传到哪了、装机、推送收不到、App 版本号、构建号、testflight release。
---
...
把 SynapseMobile 从「改完代码」送到「手机上能跑、推送能到」这条路上容易踩的坑固化下来。这里的知识有一半不在代码里，而在几个**只存在于脑子里的前提**上——最典型的是 `APNS_USE_SANDBOX`：设错了不会有任何报错，只会让推送静默消失，所以它的值只能固定，不能每次现场判断（见下文）。
...
| 包怎么装的 | token 属于哪个环境 | 服务端该配什么 |
|---|---|---|
| 数据线装（Xcode / devicectl） | sandbox | —— 收不到推送，预期如此 |
| TestFlight / App Store | 生产 | `APNS_USE_SANDBOX=false` |

device token 分环境，服务端只有**一个**开关（`server/src/mobile-live/mobile-push.service.ts:105`），所以它只能伺候一边。**网关固定走生产**：翻成 sandbox 会让所有 TestFlight 用户的推送失效，而后果不是报错，是静默失败——苹果回 `BadDeviceToken`，服务端把这个 token 当死号**永久删除且不重试**（同文件 159 行），日志里只有一句 `Mobile push token rejected`。

所以数据线装的开发包**收不到推送是接受的代价**，不要为它切网关。这是用户在 2026-09-17 定的：开发包只用来调 UI，不关心推送；要做 per-token 环境分流是另一件事，不顺手做。测试完装回 TestFlight 并**重开一次 App** 恢复——旧 token 已经被删了，不会自己回来。
...
- 改服务端配置要跑 `deploy.sh`，它会**重启生产容器、跑数据库迁移，且回滚不回滚数据库**。执行前先说清楚将改哪一行、会跑什么命令。（推送网关不在此列——它固定不变，见上文。）
...
**推送收不到（最常见，且没有任何报错）**
1. 先确认手机装的是哪种包。**装的是数据线开发包 → 这是预期行为**，不是故障：它属于 sandbox 环境，而网关固定走生产。装回 TestFlight、重开 App 即可恢复，**不要为此翻 `APNS_USE_SANDBOX`**。
2. 装的是 TestFlight 包还是收不到，才查服务端当前值：`ssh root@120.53.17.64 "grep '^APNS_USE_SANDBOX' /www/wwwroot/synapse/server/.env"`——它应该是 `false`，不是 `false` 才是问题。
3. 是 `false` 还收不到，说明不在网关这一层，别继续改它（往手机通知权限、token 有没有注册上去查）。
...
本 skill **只管**：打包上传、构建状态、群组挂载、设备安装、版本号与构建号、APNs 推送环境与验证。
