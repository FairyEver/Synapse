...
读取 Synapse Data Store 的最新代码实现，更新共享 MCP 工具定义中的 description 与 inputSchema 元数据，使 AI 编辑器（Claude Code / Cursor / Codex）能尽可能准确、完整地理解可用能力。
...
4. **共享定义优先**：当前 HTTP MCP 与 stdio MCP 共用 `desktop/data-store/shared/mcp-tools.ts`，不要在 transport 文件里复制或分叉工具描述。
...
- 避免旧公开词汇：不要把 `ENUM` / `MULTI_ENUM` / `DATETIME` / `DataStoreColumnType` 当作当前接口描述
...
2. **`desktop/electron/data-store/column-kind.ts`** — kind 与 SQLite affinity
   - `COLUMN_KINDS`
   - `ColumnKind`
   - `kindToAffinity` / `affinityToKind`
   - kind helper（choice、multi_choice、json serialized、boolean、date、timestamp）

3. **`desktop/electron/data-store/types.ts`** — 类型定义
   - `Column` / `ColumnKind`
   - `DataStoreWhereCondition` 操作符
   - `DataStoreWhereGroup`：`{ combinator: "all" | "any"; conditions: [...] }`
   - `DataStoreOrderBy` 结构
   - `DataStoreQueryParams` 参数

4. **`desktop/data-store/shared/mcp-tools.ts`** — 当前 MCP 工具定义
...
5. **transport 文件只作结构确认，不修改**
   - `desktop/data-store/shared/mcp-rpc.ts`：`tools/list` 从 `buildTools()` 返回工具列表，`tools/call` 用 `MCP_TOOL_ACTIONS` 派发
   - `desktop/data-store/mcp/index.ts`：stdio MCP bridge，只负责连接正在运行的 Synapse app
   - `desktop/electron/data-store/mcp-server.ts`：HTTP MCP server，也复用 shared mcp tools/rpc
...
编辑 `desktop/data-store/shared/mcp-tools.ts`：
...
3. 确认 `columnKindEnum` 与 `desktop/electron/data-store/column-kind.ts` 的 `COLUMN_KINDS` 一致
...
- 默认只修改 `desktop/data-store/shared/mcp-tools.ts` 中的 MCP 工具 description、inputSchema，以及相关 schema helper 常量。
- 不修改 service 层、types、CLI、HTTP server、stdio bridge、IPC handlers、preload。
- 不新增或删除工具；工具列表由 `MCP_TOOL_ACTIONS` 决定，除非用户明确要求工具级变更。
- 不修改 `desktop/data-store/shared/mcp-rpc.ts`、`desktop/data-store/mcp/index.ts`、`desktop/electron/data-store/mcp-server.ts` 的协议/transport 逻辑。
...
- **不制造第二份 MCP 描述**。HTTP MCP 和 stdio MCP 必须继续共享 `desktop/data-store/shared/mcp-tools.ts`。
- **直接修改代码**，不生成独立文件。
