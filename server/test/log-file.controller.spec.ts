...
describe("LogFileController", () => {
  ...
  it("GET /dashboard/api/logs/files returns file list", async () => {
    const res = await request(app.getHttpServer()).get("/dashboard/api/logs/files");
    expect(res.status).toBe(200);
    expect(res.body).toHaveLength(1);
    expect(res.body[0].name).toBe("server.2026-05-05.log");
  });

  it("GET /dashboard/api/logs/recent returns entries", async () => {
    const res = await request(app.getHttpServer()).get("/dashboard/api/logs/recent?limit=10");
    expect(res.status).toBe(200);
    expect(res.body.length).toBeGreaterThan(0);
  });

  it("GET /dashboard/api/logs/recent?level=error filters by level", async () => {
    const res = await request(app.getHttpServer()).get("/dashboard/api/logs/recent?level=error");
    expect(res.status).toBe(200);
    expect(res.body.every((e: { level: string }) => e.level === "error")).toBe(true);
  });

  it("GET /dashboard/api/logs/download returns zip", async () => {
    const res = await request(app.getHttpServer()).get("/dashboard/api/logs/download");
    expect(res.status).toBe(200);
