...
describe("Prisma migration deployment risk scanner", () => {
  ...
  it("fails risky pending migrations by default, with file and line details", () => {
    const { migrationsDir, appliedFile } = makeWorkspace()
    ...
    expect(result.output).toContain("Risk scan policy: failing on risky migrations")
  })
  ...
  it("continues past risky migrations only when ALLOW_RISKY_MIGRATIONS=1 is set", () => {
    const { migrationsDir, appliedFile } = makeWorkspace()
    ...
    expect(result.output).toContain("Risk scan policy: continuing because ALLOW_RISKY_MIGRATIONS=1")
  })
})
