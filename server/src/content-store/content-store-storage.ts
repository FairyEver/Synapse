...
export class LocalContentStoreStorage implements ContentStoreStoragePort {
  ...
  private pathForKey(key: string): string {
    ...
    if (objectPath !== rootPath && !objectPath.startsWith(`${rootPath}${path.sep}`)) {
      throw new Error("Invalid content store storage key.")
    }
    return objectPath
  }
}
...
export class CosContentStoreStorage implements ContentStoreStoragePort {
  ...
  private requireConfig(value: string | undefined, key: string): string {
    if (!value) throw new Error(`${key} is required for Synapse Content Store storage.`)
    return value
  }
  ...
  private getClient(): { readonly cos: COS; readonly bucket: string; readonly region: string } {
    ...
    this.client = {
      cos: new COS({
        SecretId: this.requireConfig(env.contentStoreCosSecretId, "CONTENT_STORE_COS_SECRET_ID"),
        SecretKey: this.requireConfig(env.contentStoreCosSecretKey, "CONTENT_STORE_COS_SECRET_KEY"),
      }),
      bucket: this.requireConfig(env.contentStoreCosBucket, "CONTENT_STORE_COS_BUCKET"),
      region: this.requireConfig(env.contentStoreCosRegion, "CONTENT_STORE_COS_REGION"),
    }
    return this.client
  }
}
