...
describe("server deployment configuration", () => {
  ...
  it("persists and protects local object fallback data during deployment", () => {
    ...
    expect(compose).toContain("SYNAPSE_CONTENT_STORE_LOCAL_ROOT: /app/data/content-store")
    expect(compose).toContain("- ./data/content-store:/app/data/content-store")
    ...
    expect(deployScript).toContain('CONTENT_STORE_BACKUP_FILE="$BACKUP_DIR/content-store/synapse-content-store-final-before-switch-${DEPLOY_ID}.tar.gz"')
    expect(deployScript).toContain("backup_remote_content_store_fallback")
    expect(deployScript).toContain("tar -czf \"$CONTENT_STORE_BACKUP_FILE\" -C data content-store")
    expect(deployScript).toContain("chmod 600 \"$CONTENT_STORE_BACKUP_FILE\"")
    ...
    expect(deployScript).toContain("read_env_value CONTENT_STORE_COS_SECRET_ID")
    expect(deployScript).toContain("read_env_value CONTENT_STORE_COS_SECRET_KEY")
    expect(deployScript).toContain("read_env_value CONTENT_STORE_COS_BUCKET")
    expect(deployScript).toContain("read_env_value CONTENT_STORE_COS_REGION")
