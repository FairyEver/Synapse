...
describe("server deployment configuration", () => {
  ...
  it("restores the previous service image when a stopped cutover fails", () => {
    ...
    expect(deployScript.slice(recoveryStart)).toContain("run_remote_rollback_health_check")
  })
  ...
  it("builds, isolates, health-checks, and rolls back the PDF renderer with the API", () => {
    ...
    expect(deployScript).toContain('PDF_RENDERER_ROLLBACK_STATE_FILE="$BACKUP_DIR/pdf-renderer-rollback-${DEPLOY_ID}.state"')
    expect(deployScript).toContain('printf "absent\\n" > "$PDF_RENDERER_ROLLBACK_STATE_FILE"')
    expect(deployScript).toContain("start_and_verify_new_pdf_renderer")
    expect(deployScript).toContain('container_name="synapse-pdf-renderer-preflight-${DEPLOY_ID}"')
    expect(deployScript).toContain("--network none")
    expect(deployScript).toContain('docker exec -i "$container_name" node -')
    expect(deployScript).toContain('Authorization: `Bearer ${secret}`')
    expect(deployScript).not.toContain("restore_previous_pdf_renderer")
    expect(deployScript).toContain("rm -f -s pdf-renderer")
    expect(deployScript).toContain("up -d --no-build --no-deps server")
    expect(deployScript.lastIndexOf('step 12 "预检 PDF 渲染器和数据库迁移"'))
      .toBeLessThan(deployScript.lastIndexOf("if ! run_cutover_steps; then"))
  ...
  it("documents the hardened deployment failure path", () => {
    ...
    expect(readme).toContain("失败时自动回滚 API 与原 PDF 渲染器")
  ...
  it("accepts the public update handoff only after the deployed service passes internal checks", () => {
    ...
    expect(deployScript).toContain("rollback_remote_service 2>&1 | sed 's/^/  /' && run_remote_rollback_health_check")
  ...
  it("serves drive browser pages from the console bundle and proxies direct file responses", () => {
    ...
    expect(nginx).toContain("location ~ ^/share/[^/]+(?:/items/[^/]+)?/reader$")
  ...
  it("keeps drive browser pages inside the Vite SPA while proxying direct responses", () => {
    ...
    expect(viteConfig).toContain("'^/share/[^/]+(?:/items/[^/]+)?/reader(?:\\\\?.*)?$'")
