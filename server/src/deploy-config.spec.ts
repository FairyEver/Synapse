...
describe("server deployment configuration", () => {
  ...
  it("persists and protects local object fallback data during deployment", () => {
    const deployScript = readRepoFile("deploy.sh")
    const compose = readRepoFile("server/compose.yml")

    expect(compose).toContain("SYNAPSE_DRIVE_LOCAL_ROOT: /app/data/drive")
    expect(compose).toContain("- ./data/drive:/app/data/drive")
    expect(compose).toContain("SYNAPSE_SKILL_REPOSITORY_LOCAL_ROOT: /app/data/skill-repository")
    expect(compose).toContain("- ./data/skill-repository:/app/data/skill-repository")
    ...
    expect(deployScript).toContain('SKILL_REPOSITORY_BACKUP_FILE="$BACKUP_DIR/skill-repository/synapse-skill-repository-final-before-switch-${DEPLOY_ID}.tar.gz"')
    expect(deployScript).toContain("backup_remote_skill_repository_fallback")
    expect(deployScript).toContain("tar -czf \"$SKILL_REPOSITORY_BACKUP_FILE\" -C data skill-repository")
    expect(deployScript).toContain("chmod 600 \"$SKILL_REPOSITORY_BACKUP_FILE\"")
    ...
    expect(deployScript).toContain("read_env_value SKILL_REPOSITORY_COS_SECRET_ID")
    expect(deployScript).toContain("read_env_value SKILL_REPOSITORY_COS_SECRET_KEY")
    expect(deployScript).toContain("read_env_value SKILL_REPOSITORY_COS_BUCKET")
    expect(deployScript).toContain("read_env_value SKILL_REPOSITORY_COS_REGION")
