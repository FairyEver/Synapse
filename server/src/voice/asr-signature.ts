 ...
 * 因此签名可以脱离音频流预先算好：服务端拿到 SecretKey 签出完整 URL，客户端拿
 * 这个 URL 直连腾讯云，从头到尾不接触密钥。
 *
 * 放在服务端而不是各客户端，是因为语音识别是**平台统一提供**的能力：一把密钥、
 * 一份配额，所有用户共用。客户端自己配密钥的话，每台机器都要各自申请、各自计费。
 */

export const ASR_WEBSOCKET_HOST = "asr.cloud.tencent.com"

/** 签名有效期。够一次录音握手即可，过期后客户端重新要一条。 */
