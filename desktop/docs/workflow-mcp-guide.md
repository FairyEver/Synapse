...
8. workflow_definition_inspect({ definition })          → 校验完整性
9. workflow_run_execute({ workflowId, params })         → 执行工作流
10. workflow_run_get({ runId })                         → 轮询运行结果
```

关键点：
- 步骤 3 创建的工作流已包含一个 end 节点，无需手动创建
- 步骤 5 中 position 可省略，dispatcher 自动计算布局
- 步骤 8 可在任何修改后调用，提前发现问题
- 步骤 10 需轮询直到 status 变为 `completed` / `failed` / `cancelled`
