...
MIGRATOR="$SCRIPT_DIR/import-legacy-data-store.mjs"

if [[ ! -f "$MIGRATOR" ]]; then
  echo "Error: import-legacy-data-store.mjs must be in the same folder as this script." >&2
  exit 1
fi
...
If Synapse.app is elsewhere:
  SYNAPSE_APP="/path/to/Synapse.app" ./migrate-legacy-data-store-macos.sh
EOF
exit 1
