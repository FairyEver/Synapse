...
describe("API and MCP capability surface", () => {
  it("keeps explicitly exposed capabilities mapped to only canonical MCP tool names", () => {
    ...
    expect(allCapabilityIds()).toHaveLength(239)
    expect(APP_DOMAIN.capabilities).toHaveLength(76)
    expect(buildAppTools()).toHaveLength(72)
    expect(toolNames).toHaveLength(235)
  ...
  it("keeps the clean MCP name comparison synchronized with the registry", () => {
    ...
    expect(documentedPairs).toHaveLength(145)
  })
