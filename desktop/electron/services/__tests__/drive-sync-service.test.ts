...
describe("DriveSyncService", () => {
  ...
  it("authorizes remote-to-local previews as local writes", async () => {
    const permissionGuard: PermissionGuard = {
      registerPolicy: vi.fn(),
      check: vi.fn(async () => ({ allowed: false as const, reason: "write denied by test" })),
    }
    const service = createDriveSyncService(createHarness({ permissionGuard }).deps)
    ...
    })).rejects.toThrow("write denied by test")

    expect(permissionGuard.check).toHaveBeenCalledWith(expect.objectContaining({
      action: "fs.write.outside-userdata",
      resource: "/Users/me/report.md",
      context: expect.objectContaining({ source: "driveSync.previewBinding" }),
    }))
  })
    ...
    try {
      ...
      const harness = createHarness({
        accountService: {
          ...
          listDriveItemTree: vi.fn(async () => ({ items: [], nextOffset: null })),
        },
      })
      const service = createDriveSyncService(harness.deps)
      const binding = await service.createSafeBinding({
  ...
  it("uploads local-origin files into the selected Drive parent", async () => {
    ...
    try {
      ...
      expect(listDriveItemTree).toHaveBeenCalledTimes(2)
    ...
    try {
      ...
      const harness = createHarness({
        accountService: {
          ...
          listDriveItemTree: vi.fn(async () => ({
            items: [
              { id: "remote-notes", parentId: "remote-docs", type: "folder", name: "notes", path: "Docs/notes", depth: 1, size: "0", mimeType: null, storageStatus: "active", shared: false, createdAt: "2026-06-28T00:00:00.000Z", updatedAt: "2026-06-28T00:00:00.000Z" },
              { id: "remote-spec", parentId: "remote-notes", type: "file", name: "spec.md", path: "Docs/notes/spec.md", depth: 2, size: "4", mimeType: "text/markdown", storageStatus: "active", shared: false, createdAt: "2026-06-28T00:00:00.000Z", updatedAt: "2026-06-28T00:00:00.000Z" },
            ],
            total: 2,
            fileCount: 1,
            folderCount: 1,
        ...
        accountService: {
          ...
          listDriveItemTree: vi.fn(async () => ({
            items: [
              { id: "remote-notes", parentId: "remote-docs", type: "folder", name: "notes", path: "Docs/notes", depth: 1, size: "0", mimeType: null, storageStatus: "active", shared: false, createdAt: "2026-06-28T00:00:00.000Z", updatedAt: "2026-06-28T00:00:00.000Z" },
              { id: "remote-spec", parentId: "remote-notes", type: "file", name: "spec.md", path: "Docs/notes/spec.md", depth: 2, size: "4", mimeType: "text/markdown", storageStatus: "active", shared: false, createdAt: "2026-06-28T00:00:00.000Z", updatedAt: "2026-06-28T00:00:00.000Z" },
              { id: "remote-node-modules", parentId: "remote-docs", type: "folder", name: "node_modules", path: "Docs/node_modules", depth: 1, size: "0", mimeType: null, storageStatus: "active", shared: false, createdAt: "2026-06-28T00:00:00.000Z", updatedAt: "2026-06-28T00:00:00.000Z" },
              { id: "remote-package", parentId: "remote-node-modules", type: "file", name: "package.json", path: "Docs/node_modules/package.json", depth: 2, size: "2", mimeType: "application/json", storageStatus: "active", shared: false, createdAt: "2026-06-28T00:00:00.000Z", updatedAt: "2026-06-28T00:00:00.000Z" },
            ],
            total: 4,
            fileCount: 2,
            folderCount: 2,
        ...
        accountService: {
          ...
          listDriveItemTree: vi.fn(async ({ parentId, offset }: { parentId?: string | null; offset?: number | null }) => {
            if (parentId === null || parentId === undefined) {
              rootListCalls += 1
              if (rootListCalls <= 2) return { items: [], nextOffset: null }
              return {
                items: [
  ...
  it("previews the initial local folder upload manifest", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "synapse-drive-sync-service-"))
    try {
      const localPath = path.join(tempDir, "Docs")
      await mkdir(path.join(localPath, "assets"), { recursive: true })
      await writeFile(path.join(localPath, "spec.md"), "spec", "utf8")
      await writeFile(path.join(localPath, "assets", "logo.png"), "png", "utf8")
      const service = createDriveSyncService(createHarness({
        accountService: {
          listDriveItemTree: vi.fn(async () => ({ items: [], nextOffset: null })),
        },
      ...
      expect(preview).toMatchObject({
        ...
        initialTransfer: {
          ...
          truncated: false,
          entries: [
            { action: "create_remote_folder", relativePath: ".", size: null },
            { action: "create_remote_folder", relativePath: "assets", size: null },
            { action: "upload_file", relativePath: "assets/logo.png", size: "3" },
            { action: "upload_file", relativePath: "spec.md", size: "4" },
          ],
        },
      })
    } finally {
  ...
  it("previews the initial remote folder download manifest", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "synapse-drive-sync-service-"))
    try {
      const service = createDriveSyncService(createHarness({
        accountService: {
      ...
      await expect(service.previewBinding({
        ...
        initialTransfer: {
          ...
          truncated: false,
          entries: [
            { action: "create_local_folder", relativePath: ".", size: null },
            { action: "create_local_folder", relativePath: "assets", size: null },
            { action: "download_file", relativePath: "assets/logo.png", size: "3" },
            { action: "download_file", relativePath: "spec.md", size: "4" },
          ],
        },
      })
    } finally {
  ...
  it("caps initial transfer preview entries without losing totals", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "synapse-drive-sync-service-"))
    try {
      const localPath = path.join(tempDir, "Docs")
      await mkdir(localPath)
      await Promise.all(Array.from({ length: 201 }, (_, index) => (
        writeFile(path.join(localPath, `file-${String(index).padStart(3, "0")}.txt`), "x", "utf8")
      )))
      const service = createDriveSyncService(createHarness({
        accountService: {
          listDriveItemTree: vi.fn(async () => ({ items: [], nextOffset: null })),
      ...
      expect(preview).toMatchObject({
        ...
        initialTransfer: {
          ...
          folderCount: 1,
          totalBytes: "201",
          entries: expect.arrayContaining([{ action: "create_remote_folder", relativePath: ".", size: null }]),
          truncated: true,
        },
      })
      expect(preview.initialTransfer?.entries).toHaveLength(200)
    } finally {
      await rm(tempDir, { recursive: true, force: true })
    }
  })
  ...
  it("rejects local file uploads when the target parent already contains the same name", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "synapse-drive-sync-service-"))
    try {
      const localPath = path.join(tempDir, "Spec.md")
      await writeFile(localPath, "local", "utf8")
      const uploadDriveSyncFile = vi.fn(async () => mockDriveItem("unexpected-upload"))
      const harness = createHarness({
        accountService: {
          uploadDriveSyncFile,
          listDriveItemTree: vi.fn(async () => ({
            items: [
              { id: "existing-spec", parentId: "target-parent", name: "spec.md", type: "file", path: "spec.md", depth: 0 },
            ],
            nextOffset: null,
          })),
        },
      ...
      await expect(service.previewBinding({
        ...
        status: "blocked",
        direction: null,
        reason: expect.stringContaining("目标云盘位置已存在同名文件"),
      })
  ...
  it("stops a full rescan instead of materializing cloud ancestor paths when the binding hint is a bare name", async () => {
    ...
    const fileNames = ["测试清单.md", "功能变更清单.md", "产品设计文档.md"]
    try {
      const localRoot = path.join(tempDir, leaf)
      await mkdir(localRoot, { recursive: true })
      for (const name of fileNames) await writeFile(path.join(localRoot, name), `content of ${name}`, "utf8")

      const remoteItems = fileNames.map((name, index) => ({
        id: `remote-file-${index}`,
        parentId: "remote-root",
        name,
        type: "file",
        // The real server returns the absolute path from the drive root, ancestor chain included.
        path: `${cloudChain}/${name}`,
        size: String(Buffer.byteLength(`content of ${name}`)),
        depth: 1,
      }))
      const harness = createHarness({
        accountService: {
          getDriveItem: vi.fn(async () => ({ ...mockDriveItem("remote-root"), type: "folder" as const, name: leaf, size: "0" })),
          listDriveItemTree: vi.fn(async () => ({ items: remoteItems })),
          downloadDriveFile: vi.fn(async ({ itemId, outputPath }: { itemId: string; outputPath: string }) => {
            const index = Number(itemId.replace("remote-file-", ""))
            await writeFile(outputPath, `content of ${fileNames[index]}`, "utf8")
            return { ok: true as const, path: outputPath }
          }),
        },
      })
      ...
      const binding = await service.createBinding({
        ...
        driveItemName: leaf,
        // Bare leaf name, exactly as the MCP/http dispatcher writes it.
        drivePathHint: leaf,
        kind: "folder",
        localPath: localRoot,
        remoteCursor: "100",
      ...
      for (const [index, name] of fileNames.entries()) {
        const stats = await lstat(path.join(localRoot, name))
        await harness.baseline.upsert({
          id: `${binding.id}:${name}`,
          schemaVersion: 1,
          bindingId: binding.id,
          relativePath: name,
          kind: "file",
          remoteItemId: `remote-file-${index}`,
          remoteVersionId: null,
          remoteEtag: null,
          localSize: stats.size,
          localMtimeMs: stats.mtimeMs,
          localHash: await hashDriveSyncFile(path.join(localRoot, name)),
          lastSyncedAt: "2026-06-28T00:00:00.000Z",
          deletedAt: null,
        })
      }
  ...
  it("keeps a full rescan unchanged when the binding hint is an absolute cloud path", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "synapse-drive-sync-service-"))
    const leaf = "流程-撤销已通过审批"
    const cloudChain = `公司文档镜像/系统/平台/设计文档/${leaf}`
    const fileNames = ["测试清单.md", "功能变更清单.md", "产品设计文档.md"]
