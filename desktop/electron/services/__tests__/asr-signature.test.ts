...
describe("buildSignatureSource", () => {
  ...
  it("不把 signature 自身算进签名原文", () => {
    const source = buildSignatureSource("1", { a: "1", signature: "ignored" })
    expect(source).toBe("asr.cloud.tencent.com/asr/v2/1?a=1")
  })
})
...
describe("送包常量", () => {
  it("200ms 的 16k 单声道 PCM 是 6400 字节", () => {
    expect(ASR_CHUNK_BYTES).toBe(6400)
  })
})
