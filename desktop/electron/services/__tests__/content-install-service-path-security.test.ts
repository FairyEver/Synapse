...
vi.mock("electron", () => ({
  app: {
    getPath: (which: string) => `/tmp/synapse-content-install-path-test-${which}`,
...
async function createTempRoot(): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "synapse-content-install-path-"))
  tempRoots.push(root)
  return root
}
...
describe("ContentInstallService path security", () => {
  ...
  it("rejects install form reads outside configured editor rule roots", async () => {
    ...
    await expect(contentInstallService.readEditorInstallFormValues({
      editorId: "test-editor",
