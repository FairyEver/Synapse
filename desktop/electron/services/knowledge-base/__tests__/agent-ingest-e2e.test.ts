...
async function runFakeSdkIngestTurn(projectPath: string, content: "/wiki ingest" | "汲取知识"): Promise<void> {
  ...
  const agentFacingPrompt = content === "/wiki ingest"
    ? expectObjectOutput(await contribution?.commands[0]?.buildPrompt(["ingest"], message), "prompt")
    : (await Promise.resolve(contribution?.prepareMessage?.(message, { isNewLiveSession: true })))?.content

  expect(agentFacingPrompt).toContain(".raw/note.md")
  expect(agentFacingPrompt).toContain("## Synapse 预检")
