...
describe("buildServiceRegistry (T1.8)", () => {
  ...
  it("registers all bootstrap services with correct dependsOn graph", async () => {
    ...
    expect(byId.get("git.repository-registry")?.dependsOn).toEqual(["git.command-runner"])
    expect(byId.get("git.environment-service")?.dependsOn).toEqual(["git.command-runner", "core.process-environment"])
    expect(byId.get("git.clone-service")?.dependsOn).toEqual(["git.command-runner", "git.repository-registry", "core.data-repository"])
    ...
    expect(byId.get("git.commit-service")?.dependsOn).toEqual([
      "git.command-runner",
      "git.change-selection-service",
      "git.status-service",
    ])
    ...
    expect(byId.get("core.update")?.dependsOn).toEqual([
      "core.config",
      "core.window-manager",
      "core.permission-guard",
      "core.audit-sink",
