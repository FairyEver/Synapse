...
import type { DatabaseCliDebugInfo, DatabaseCliStatus } from "../../src/types/database"
import { createMainLogger } from "../services/log-store"

const logger = createMainLogger("database.cli-installer")
...
function getMcpScriptPath(): string {
  if (app.isPackaged) {
    return path.join(process.resourcesPath, "database", "mcp", "index.js")
  }
  return path.join(app.getAppPath(), "dist-database", "mcp", "index.js")
}

function getBundledCliScriptPath(): string {
  if (app.isPackaged) {
    return path.join(process.resourcesPath, "database", "cli", "index.js")
  }
  return path.join(app.getAppPath(), "dist-database", "cli", "index.js")
}
...
async function ensureCliBundleReady(): Promise<void> {
  ...
  logger.info("Building database CLI bundle for development install.", {
    cwd: app.getAppPath(),
  })

  await execFileAsync(PNPM_COMMAND, ["build:database"], {
    cwd: app.getAppPath(),
    timeout: 120000,
  })
}
...
async function getCliStatus(): Promise<DatabaseCliStatus> {
...
async function getCliDebugInfo(): Promise<DatabaseCliDebugInfo> {
  const status = await getCliStatus()
