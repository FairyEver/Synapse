...
export type KnowledgeBaseIngestTurnRecordV1 =
  | {
    readonly kind: "preflight"
    readonly state: Record<string, unknown>
  }
  | {
    readonly kind: "no-finalize"
    readonly reason: "direct-result"
  }

export interface KnowledgeBasePendingIngestRecoveryV1 extends Record<string, unknown> {
  projectPath: string
  conversationId: string
  turnId: string
  failedAt: string
...
export type KnowledgeBaseIngestTurnEntryV1 =
  | {
    readonly id: string
    ...
    readonly record: KnowledgeBaseIngestTurnRecordV1
  }
  | {
    readonly id: string
    readonly schemaVersion: 1
    ...
    readonly turnId: string
    readonly recovery: KnowledgeBasePendingIngestRecoveryV1
  }

export const knowledgeBaseIngestTurnsSchema: NamespaceSchema<KnowledgeBaseIngestTurnEntryV1> = {
  name: "knowledge-base.ingest-turns",
  backend: "sqlite",
  currentVersion: 1,
  migrations: noMigrations,
  validate: (v): v is KnowledgeBaseIngestTurnEntryV1 =>
    isAnyRecord<KnowledgeBaseIngestTurnEntryV1>(v)
    && (v as KnowledgeBaseIngestTurnEntryV1).schemaVersion === 1
    && typeof (v as KnowledgeBaseIngestTurnEntryV1).id === "string"
    && typeof (v as KnowledgeBaseIngestTurnEntryV1).projectId === "string"
    && typeof (v as KnowledgeBaseIngestTurnEntryV1).turnId === "string"
    && (
      ((v as KnowledgeBaseIngestTurnEntryV1).entryType === "turn"
        && isKnowledgeBaseIngestTurnRecord((v as Extract<KnowledgeBaseIngestTurnEntryV1, { entryType: "turn" }>).record))
      || ((v as KnowledgeBaseIngestTurnEntryV1).entryType === "recovery"
        && isKnowledgeBasePendingIngestRecovery((v as Extract<KnowledgeBaseIngestTurnEntryV1, { entryType: "recovery" }>).recovery))
    ),
}

function isKnowledgeBaseIngestTurnRecord(value: unknown): value is KnowledgeBaseIngestTurnRecordV1 {
  if (!isPlainRecord(value)) return false
  if (value.kind === "no-finalize") {
    return value.reason === "direct-result"
  }
  return value.kind === "preflight" && isPlainRecord(value.state)
}

function isKnowledgeBasePendingIngestRecovery(value: unknown): value is KnowledgeBasePendingIngestRecoveryV1 {
  return isPlainRecord<KnowledgeBasePendingIngestRecoveryV1>(value)
