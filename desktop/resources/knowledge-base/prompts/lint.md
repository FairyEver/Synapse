...
Synapse 会在本提示后附加“确定性预检”结果。那些结果来自 Synapse 内部服务，是 address、manifest、tiling 等机械检查的事实来源。
...
不要运行、引用或要求用户 vault 中存在 DragonScale 脚本；Synapse 已经内部化这些机制。不要编造 address validation 或 semantic tiling 结果。
