---
type: raw-research
title: "Synapse Knowledge Base Ecosystem Research"
date: 2026-04-08
queries:
...
> Researched: 2026-04-08 | Goal: cherry-pick best features for AgriciDaniel/Synapse Knowledge Base
...
### AgriciDaniel/Synapse Knowledge Base ⭐ OUR PROJECT
- URL: https://github.com/AgriciDaniel/Synapse Knowledge Base
- Version: 1.2.0
...
Most mature projects (Ar9av/obsidian-wiki) use a `.manifest.json` to track ingested sources — hash, timestamp, which pages produced. Re-ingest only processes changed/new files. Synapse Knowledge Base currently has no delta tracking.

### Pattern 2: Multi-Depth Queries
rvk7895 implements 3 tiers: Quick (index only), Standard (wiki + web), Deep (parallel agents). Current Synapse Knowledge Base has one depth level in wiki-query.

### Pattern 3: Goal Cascade Integration
ballred's project connects personal productivity (daily/weekly reviews) with the knowledge base. No PKM projects do this today in Synapse Knowledge Base.
...
Ar9av's setup.sh deploys skills to Claude Code + Cursor + Windsurf + Codex + Gemini CLI simultaneously. Synapse Knowledge Base is Claude Code only.
...
Ar9av's wiki has no fixed structure upfront — it emerges from the content. Synapse Knowledge Base has a predefined structure.

### Pattern 8: Vision Ingestion
Ar9av supports images/screenshots/whiteboards as ingestable sources (vision model required). Synapse Knowledge Base has no image ingestion.

### Pattern 9: Output Formats
rvk7895 and ekadetov export to Marp slides and matplotlib charts. Synapse Knowledge Base outputs only Markdown.

### Pattern 10: Vault Adoption
Both claudesidian and ballred/pkm can be adopted onto existing vaults without destroying structure. Synapse Knowledge Base requires starting fresh.
...
- https://github.com/AgriciDaniel/Synapse Knowledge Base
