# Synapse Knowledge Base Hooks

Plugin hooks for the Synapse Knowledge Base wiki vault. All hooks are defined in `hooks.json`.
...
| `PreCompact` | command + prompt | Loads `wiki/hot.md` into the compaction context before summarization. Hook-injected context does NOT survive compaction (only `CLAUDE.md` does), so this hook makes the hot cache part of the compacted session state. |
...
**Impact**: If this bug is active in your Claude Code version, the prompt-type SessionStart and PreCompact hooks may not inject context as expected.

**Workaround**: The command-type SessionStart hook (`cat wiki/hot.md`) is the canonical safety check. The PreCompact hook also emits `wiki/hot.md` before summarization. Both rely on STDOUT capture for context injection, so test against this issue if hot cache restoration fails. As a fallback, copy the hook config from `hooks.json` into your user-level `~/.claude/settings.json` instead of relying on plugin hooks.
