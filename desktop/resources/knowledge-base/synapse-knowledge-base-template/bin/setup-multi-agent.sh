#!/usr/bin/env bash
# Synapse Knowledge Base: multi-agent skill installer
...
#   - Codex CLI      : symlink to ~/.codex/skills/Synapse Knowledge Base
#   - OpenCode       : symlink to ~/.opencode/skills/Synapse Knowledge Base
#   - Gemini CLI     : symlink to ~/.gemini/skills/Synapse Knowledge Base
...
if [ ! -d "$SKILLS_DIR" ]; then
  echo "ERROR: $SKILLS_DIR does not exist. Are you running this from the Synapse Knowledge Base repo?"
  exit 1
fi
...
echo "Synapse Knowledge Base: multi-agent skill installer"
echo "Repo: $REPO_ROOT"
echo

# Codex CLI
link_if_missing "$SKILLS_DIR" "$HOME/.codex/skills/Synapse Knowledge Base" "Codex CLI"

# OpenCode
link_if_missing "$SKILLS_DIR" "$HOME/.opencode/skills/Synapse Knowledge Base" "OpenCode"

# Gemini CLI
link_if_missing "$SKILLS_DIR" "$HOME/.gemini/skills/Synapse Knowledge Base" "Gemini CLI"
...
echo "  - Codex CLI:   codex --list-skills | grep Synapse Knowledge Base"
