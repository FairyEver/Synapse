---
type: session
title: "Synapse Knowledge Base v1.4 Release Session"
created: 2026-04-08
updated: 2026-04-08
tags:
  - meta
...
status: evergreen
related:
  - "[[Synapse Knowledge Base-ecosystem]]"
  - "[[cherry-picks]]"
  - "[[full-audit-and-system-setup-session]]"
  - "[[Synapse Knowledge Base-v1.2.0-release-session]]"
  - "[[LLM Wiki Pattern]]"
sources:
  - "[[Synapse Knowledge Base-ecosystem-research]]"
---

# Synapse Knowledge Base v1.4 Release Session
...
Shipped in response to an internal quality check against the wider ecosystem (16+ Claude plus Obsidian projects researched, filed in [[Synapse Knowledge Base-ecosystem]]). The highest-value features from competing implementations were cherry-picked and shipped as v1.1.
...
| `.cursor/rules/Synapse Knowledge Base.mdc` | Cursor (always-on rules) |
| `.windsurf/rules/Synapse Knowledge Base.md` | Windsurf Cascade |
| `.github/copilot-instructions.md` | GitHub Copilot |
| `bin/setup-multi-agent.sh` | Idempotent symlink installer that wires up `skills/` into each agent's expected location |

This turns Synapse Knowledge Base into a multi-agent plugin at near-zero compatibility cost. Pattern borrowed from [[Ar9av-obsidian-wiki]] which was the reference implementation for multi-agent support.
...
```bash
claude plugin install github:AgriciDaniel/Synapse Knowledge Base
```

This form does not exist in Claude Code. Users trying it see:

```
Failed to install plugin "github:AgriciDaniel/Synapse Knowledge Base": Plugin "github:AgriciDaniel/Synapse Knowledge Base" not found in any configured marketplace
```
...
claude plugin marketplace add AgriciDaniel/Synapse Knowledge Base

# Step 2: install the plugin from the catalog by name
claude plugin install synapse-knowledge-base
```

Where `Synapse Knowledge Base` is the plugin name (from `plugin.json`) and `synapse-knowledge-base` is the marketplace name (from `marketplace.json`). The `@` delimiter separates them.

### Why the confusion existed

There is no `claude plugin install github:owner/repo` shortcut. The marketplace abstraction is mandatory: Claude Code always fetches via a registered marketplace. A single-repo plugin like Synapse Knowledge Base is both the marketplace host and the plugin host, and the user must register the marketplace first before installing any plugin from it.
...
8. **Marketplace name and plugin name can differ**. Our marketplace is `synapse-knowledge-base`, our plugin is `Synapse Knowledge Base`. The `@` delimiter disambiguates them.
...
| `wiki/comparisons/Synapse Knowledge Base-ecosystem.md` | comparison | 16+ project feature matrix |
| `wiki/concepts/cherry-picks.md` | concept | Prioritized feature backlog |
| `wiki/sources/Synapse Knowledge Base-ecosystem-research.md` | source | Research summary |
...
| `.raw/Synapse Knowledge Base-ecosystem-research.md` | raw source | Ecosystem research dump |
| `hooks/README.md` | doc | Hook documentation |
| `AGENTS.md` | bootstrap | Codex CLI / OpenCode |
| `GEMINI.md` | bootstrap | Gemini CLI / Antigravity |
| `.cursor/rules/Synapse Knowledge Base.mdc` | bootstrap | Cursor rules |
| `.windsurf/rules/Synapse Knowledge Base.md` | bootstrap | Windsurf Cascade |
| `.github/copilot-instructions.md` | bootstrap | GitHub Copilot |
| `bin/setup-multi-agent.sh` | script | Multi-agent symlink installer |

## Current Plugin State

- **Plugin installed**: `synapse-knowledge-base` version `1.4.1`, user scope, enabled
