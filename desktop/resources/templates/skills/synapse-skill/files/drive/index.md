...
7. If the user asks to upload to `公开素材`, upload to a `图床`, generate a `直链`, generate an `外链`, create a `public asset`, or create a `direct link`, call `app_drive_direct_link_upload`. Public assets are image-only in v1, support PNG/JPG/JPEG/GIF/WebP/AVIF/ICO, do not support SVG, are flat, and duplicate names are allowed; every upload creates a new asset id and `/files/<assetId>` URL.
...
11. If the user asks to publish a Drive folder as a static website, folder site, multi-page HTML prototype, or product prototype site, call `app_drive_site_create`. Sites use `/sites/<siteId>/`, copy the folder at publish time, and do not grant Drive browse or edit access.
   ...
   - Use `accessMode: "public"` for open sites or `accessMode: "password"` when the user asks for a password. Pass `password` only when the user provides a custom site password; omit it to let Synapse generate one.
   ...
   - Use the `app_drive_site_create` or `app_drive_site_update_access` result when the user needs the new site password. `app_drive_site_list` lists existing sites without returning passwords.
