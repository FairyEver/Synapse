...
const TEXT_FILE_WRITER_WORKFLOW_GUIDANCE = "The text_file_writer_file_write node writes one complete text string to an absolute local .txt, .md, .csv, .html, or .htm path. HTML and HTM require utf8. It uses path, text, encoding, overwrite, and variables; path and text support explicit {{variable}} interpolation, there is no separate format field or text length limit, and incoming edges never supply implicit text."
        ...
        properties: {
          ...
          config: {
            ...
            properties: {
              ...
              path: { type: "string", description: "file_opener_file_open/text_file_writer_file_write: absolute local path. The writer accepts .txt, .md, .csv, .html, or .htm. Supports {{variable}} interpolation." },
