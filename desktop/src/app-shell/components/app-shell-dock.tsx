...
export function AppShellDock({ apps, value, onValueChange }: AppShellDockProps) {
  return (
    <TooltipProvider>
      <nav data-track="app-shell-dock" className="flex min-w-0 justify-center">
                ...
                <Tooltip key={app.id}>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      className="relative hover:bg-transparent active:bg-transparent aria-[current=page]:bg-transparent aria-[current=page]:text-foreground dark:hover:bg-transparent"
                      aria-label={app.name}
