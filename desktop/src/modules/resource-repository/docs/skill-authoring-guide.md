...
在 Skill 根目录使用标准 `.env.example`，键名必须匹配 `[A-Za-z_][A-Za-z0-9_]*`，并应稳定、清晰。必填值留空，非敏感配置可以提供默认值。
...
扫描支持 Windows，但当前 Windows 不执行安全原子队列写入，选中项会逐项返回失败；用户需手动更新本地 `.env`。macOS 和 Linux 会在最终校验后，通过同目录原子替换写入每一项。

运行时 `.env` 的扫描、重新安装合并和队列更新大小上限均为 1 MiB；超限时安全失败，不读取或覆盖该文件。
...
3. 在 Skill 根目录创建或完善 `.env.example`。使用标准 Dotenv `KEY=value` 格式；键名必须匹配 `[A-Za-z_][A-Za-z0-9_]*`；只写键名、必要注释和非敏感默认值，不写真实 token、密码、私钥或生产连接信息。
...
3. 只有 Skill 确实需要外部配置时才在根目录创建 `.env.example`。使用标准 Dotenv `KEY=value` 格式；键名必须匹配 `[A-Za-z_][A-Za-z0-9_]*`，并应稳定、清晰；只包含必要注释和非敏感默认值。
