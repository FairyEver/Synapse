...
function formatCliDebugInfo(debugInfo: DataStoreCliDebugInfo): string {
  return JSON.stringify(debugInfo, null, 2)
}

function formatCliDetailsForCopy(debugInfo: DataStoreCliDebugInfo): string {
  return [
    "CLI 详细信息",
    "",
...
function getCliStatusLabel(status: DataStoreCliStatus | null): string {
...
function DataStoreSettingsPanel() {
  ...
  const [cliStatus, setCliStatus] = useState<DataStoreCliStatus | null>(null)
  const [cliDebugInfo, setCliDebugInfo] = useState<DataStoreCliDebugInfo | null>(null)
  ...
  const refreshCliDebugInfo = useCallback(async () => {
    ...
    try {
      const result = await getCliDebugInfo()
