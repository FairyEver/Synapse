...
function generateMCPExample(schema: DatabaseTableSchema): string {
  ...
  lines.push(
    ...
    "```",
    ``,
    `### \`database_row_list\`（可选 \`where\` / \`orderBy\` / \`limit\` / \`offset\`，默认上限 100 行）`,
    "```json",
    stringifyJson({ tableName: schema.name }),
    "```",
    ``,
    `### \`database_row_update\`（按 id 部分更新）`,
    "```json",
    stringifyJson({ tableName: schema.name, rowId: 1, data: sampleRow }),
    "```",
    ``,
    `### \`database_row_delete\`（按 id）`,
    "```json",
...
function generateSkillContext(schema: DatabaseTableSchema): string {
  ...
  lines.push(
    ...
    `- \`updated_at\` (timestamp) — 更新时间，每次 database_row_update 自动刷新`,
    ``,
    `### 业务列`,
    ...formatColumnLines(editableCols),
    ``,
    ...
    ``,
    `**读取**`,
    `- \`database_row_list\` — 条件查询 \`{ tableName, where?, orderBy?, limit?, offset? }\`（默认上限 100 行）`,
    `- \`database_row_count\` — 计数 \`{ tableName, where? }\``,
    `- \`database_table_describe\` — 查看最新结构 \`{ tableName }\`（确认 choices 当前值）`,
    ``,
    `**写入**`,
    `- \`database_row_create\` — 插入一行 \`{ tableName, data }\`，返回 \`{ id }\``,
    `- \`database_rows_create\` — 事务性批量插入 \`{ tableName, rows }\`，返回 \`{ ids }\``,
    `- \`database_row_update\` — 按 id 部分更新 \`{ tableName, rowId, data }\``,
    `- \`database_rows_update\` — 按条件批量更新 \`{ tableName, where, data }\`（where 不能为空）`,
    `- \`database_row_delete\` — 按 id 删除 \`{ tableName, rowId }\``,
    `- \`database_rows_delete\` — 按条件批量删除 \`{ tableName, where }\`（where 不能为空）`,
    ``,
    `## 字段取值规则`,
    ``,
    ...
    `## 调用示例`,
    ``,
    `### 插入一行 · \`database_row_create\``,
    "```json",
    stringifyJson({ tableName: schema.name, data: sampleRow }),
    "```",
    ``,
    `### 查询全部 · \`database_row_list\``,
    "```json",
    stringifyJson({ tableName: schema.name }),
    "```",
    ``,
  ...
  if (singleChoiceCol && singleChoiceCol.choices) {
    lines.push(
      `### 按 \`${singleChoiceCol.name}\` 过滤 · \`database_row_list\``,
      "```json",
      stringifyJson({
  ...
  if (multiChoiceCol && multiChoiceCol.choices) {
    lines.push(
      `### 按 \`${multiChoiceCol.name}\` 包含过滤 · \`database_row_list\``,
      "```json",
      stringifyJson({
        tableName: schema.name,
        where: [
  ...
  lines.push(
    `### 按 id 更新 · \`database_row_update\``,
    "```json",
    stringifyJson({ tableName: schema.name, rowId: 1, data: sampleRow }),
    "```",
    ``,
    `### 按 id 删除 · \`database_row_delete\``,
    "```json",
    stringifyJson({ tableName: schema.name, rowId: 1 }),
    "```",
  )
