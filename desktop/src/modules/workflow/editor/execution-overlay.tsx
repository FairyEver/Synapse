  ...
  return (
    <>
      <div className="absolute bottom-4 right-4 bg-background/90 border rounded-lg shadow-sm p-3 flex flex-col gap-1.5 max-h-64 overflow-auto z-10">
        ...
        {runState in RUN_STATE_BADGE && (
          <div className="flex flex-col gap-1 pb-1.5 mb-0.5 border-b">
            <Badge variant={RUN_STATE_BADGE[runState as WorkflowRunStatus["status"]].variant} className="text-xs w-fit">{RUN_STATE_BADGE[runState as WorkflowRunStatus["status"]].label}</Badge>
        ...
        {results.map((r) => (
          ...
          >
            <Badge variant={NODE_STATUS_VARIANT[r.status] ?? "outline"} className="text-xs shrink-0">{NODE_STATUS_LABEL[r.status] ?? r.status}</Badge>
