...
function AgentModule({ pendingAgentSession, onPendingAgentSessionConsumed }: AgentModuleProps) {
  ...
  const sidebar = (
    <AgentSessionSidebar
      ...
      onCreateSession={async (projectId, selection, name, personaId) => {
      ...
      onDelete={(session) => chat.deleteSession(session)}
      onDeleteOthers={async (keep, groupSessions) => {
        const others = groupSessions.filter(
