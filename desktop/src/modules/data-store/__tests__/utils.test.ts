...
import type { DataStoreTableSchema } from "@/types/data-store"

const customerSchema: DataStoreTableSchema = {
  name: "customers",
  description: "",
...
describe("data-store utils", () => {
  it("hides stale query and schema state for a newly selected table", () => {
    expect(getCurrentDataStoreQueryResult("orders", {
      table: "customers",
      data: { rows: [{ id: 1, name: "Ada" }], total: 1 },
      error: null,
    })).toEqual({ rows: [], total: 0 })

    expect(getCurrentDataStoreSchema("orders", {
      table: "customers",
  ...
  it("keeps errors scoped to the table that produced them", () => {
    ...
    expect(getCurrentDataStoreError("customers", { table: "customers", data: { rows: [], total: 0 }, error }))
      .toBe(error)
    expect(getCurrentDataStoreError("orders", { table: "customers", data: { rows: [], total: 0 }, error }))
      .toBeNull()
  })
