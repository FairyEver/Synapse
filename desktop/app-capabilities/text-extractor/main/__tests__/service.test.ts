...
function createTestService(logger?: { info: ReturnType<typeof vi.fn>; warn: ReturnType<typeof vi.fn> }) {
  ...
  return {
    service: createTextExtractorService({
...
function createWorkerBoundary(mode: "controlled" | "memory" = "controlled") {
  ...
  const factory: TextExtractionWorkerFactory = (_filename, options) => {
...
function createSchedulingTestService(workerFactory: TextExtractionWorkerFactory) {
...
describe("TextExtractorService", () => {
  it("extracts DOCX paragraphs, list text, table cells, and a text box through the real worker", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "synapse-text-extractor-"))
  ...
  it("keeps a task waiting while bytes are prepared and starts it after Worker creation", async () => {
    ...
    const workerFactory: TextExtractionWorkerFactory = (_filename, options) => {
  ...
  it("launches the worker with bytes-only input, transfer ownership, and bounded memory", async () => {
    ...
    let capturedOptions: Parameters<TextExtractionWorkerFactory>[1] | undefined
    const workerFactory: TextExtractionWorkerFactory = (filename, options) => {
