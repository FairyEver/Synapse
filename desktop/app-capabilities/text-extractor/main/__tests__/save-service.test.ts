...
describe("TextSaveService", () => {
  ...
  it("safely replaces a regular file selected through the system dialog", async () => {
    ...
    await createTextSaveService().save({ outputPath, text: "新正文" })
  ...
  it("rejects symbolic links and non-TXT output paths", async () => {
    ...
    await expect(createTextSaveService().save({ outputPath: linkedPath, text: "覆盖" }))
      .rejects.toMatchObject({
        code: "UNSAFE_OUTPUT_TARGET",
        message: "无法安全写入所选文件。",
      })
    await expect(readFile(targetPath, "utf8")).resolves.toBe("保留")
    await expect(createTextSaveService().save({
...
async function createTempDirectory(): Promise<string> {
  const directory = await mkdtemp(path.join(os.tmpdir(), "synapse-text-extractor-save-"))
