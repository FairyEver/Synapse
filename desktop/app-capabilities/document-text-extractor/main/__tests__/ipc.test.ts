...
describe("documentTextExtractorIpcModule", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it("declares the document extraction IPC surface and validates requests", () => {
    const module = createDocumentTextExtractorIpcModule()

    expect(module.id).toBe("documentTextExtractor")
    expect(module.methods.chooseDocument.operationId).toBe("app.document_text_extractor.document.choose")
    expect(module.methods.extractDocument.operationId).toBe("app.document_text_extractor.document.extract")
    expect(module.methods.cancelExtraction.operationId).toBe("app.document_text_extractor.operation.cancel")
    expect(module.methods.chooseOutput.operationId).toBe("app.document_text_extractor.output.choose")
    expect(module.methods.saveText.operationId).toBe("app.document_text_extractor.text.save")
    expect(module.events.status.operationId).toBe("app.document_text_extractor.operation.status")
  ...
  it("broadcasts waiting/running states and returns the shared result", async () => {
    ...
    await expect(module.methods.extractDocument.handler(createContext({
      "core.document-text-extractor": { createTask },
  ...
  it("cancels the active extraction and serializes stable errors", async () => {
    ...
    const cancel = vi.fn(() => {
      rejectResult(new DocumentTextExtractionError("EXTRACTION_CANCELLED"))
      return true
    })
    const task = {
      result,
    ...
    const context = createContext({
      "core.document-text-extractor": { createTask: () => task },
  ...
  it("serializes synchronous extraction validation failures without exposing the path", async () => {
    ...
    const response = await module.methods.extractDocument.handler(createContext({
      "core.document-text-extractor": {
        createTask: () => { throw new DocumentTextExtractionError("UNSUPPORTED_FORMAT") },
      },
  ...
  it("checks write permission, records a path-safe audit, and saves full UTF-8 text", async () => {
    ...
    const module = createDocumentTextExtractorIpcModule({ saveService: { save } })
