...
vi.mock("@/lib/electron-bridge", () => ({
  requireBridgeDomain: (domain: string) => {
    if (domain === "documentTextExtractor") return bridge
...
import { DocumentTextExtractorModule } from "../index"
...
describe("DocumentTextExtractorModule", () => {
  it("waits for an explicit action after choosing a document", async () => {
    ...
    expect(document.querySelector<HTMLInputElement>("#document-text-extractor-path")?.value)
      .toBe("/tmp/report.pdf")
  ...
  it("shows waiting and running states and cancels without an error toast", async () => {
    ...
    expect(document.querySelector<HTMLInputElement>("#document-text-extractor-path")?.disabled).toBe(true)
    ...
    await act(async () => {
      resolveExtraction({
        ok: false,
        error: { code: "EXTRACTION_CANCELLED", message: "文档文本提取已取消。" },
      })
      await Promise.resolve()
    })
  })
  ...
  it("keeps the single-task UI keyboard and screen-reader accessible", () => {
    ...
    const input = document.querySelector<HTMLInputElement>("#document-text-extractor-path")
    expect(input?.readOnly).toBe(true)
    expect(document.querySelector("label[for='document-text-extractor-path']")).toBeInstanceOf(HTMLLabelElement)
...
function renderModule(): Root {
  ...
  roots.push(root)
  act(() => root.render(<DocumentTextExtractorModule />))
  return root
}
