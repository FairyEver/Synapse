...
import { fileOpenerNodeManifest } from "./manifest"
import type { FileOpenerNodeConfig } from "./schema"

export function FileOpenerNodeCard({ config, name, selected, status, progressLabel, startedAt, nodeId }: {
  config: FileOpenerNodeConfig
  name?: string
  selected?: boolean
  status?: NodeStatus
  ...
  const Icon = fileOpenerNodeManifest.icon
  ...
  return (
    <div className={cn(
      ...
      <div className="mb-1.5 flex items-center gap-2">
        ...
        <span className="min-w-0 flex-1 truncate text-xs font-medium text-foreground">{name || "默认应用打开"}</span>
      ...
      <p className="truncate text-[11px] text-muted-foreground">
        {status === "running" && progressLabel ? progressLabel : config.path || "未设置文件路径"}
      </p>
