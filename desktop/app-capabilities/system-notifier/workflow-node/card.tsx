...
import { systemNotifierNodeManifest } from "./manifest"
import type { SystemNotifierNodeConfig } from "./schema"

export function SystemNotifierNodeCard({
  config,
  name,
  selected,
  status,
  ...
  config: SystemNotifierNodeConfig
  name?: string
  selected?: boolean
  status?: NodeStatus
  ...
  const Icon = systemNotifierNodeManifest.icon
  ...
  return (
    <div className={cn(
      ...
      <p className="truncate text-[11px] text-muted-foreground">
        {status === "running" && progressLabel ? progressLabel : config.title || "未设置标题"}
      </p>
