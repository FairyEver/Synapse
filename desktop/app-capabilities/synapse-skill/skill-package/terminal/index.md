...
- `SYNAPSE_SESSION_ID` and `SYNAPSE_WORKSPACE_ID` are this process's session and its tab, present in every terminal session; with neither present, this process is not inside a Synapse terminal.
- `SYNAPSE_TERMINAL_SESSION_ID` and the `SYNAPSE_TERMINAL_AGENT_*` values are a second set carrying the same session id; they belong to the agent-hook runtime and appear only when Agent notifications are on. Two session ids in the environment is expected. Never echo an agent token.

Finding the target:

- A pasted reference — `workspace_id`, `workspace_title`, `session_id`, `session_title`, `session_ref` — is the one thing in the message that names a terminal; the titles are there so you can say which one you mean. A phrase like "this terminal", "this conversation", or "here" names nothing you can see: you cannot tell which one the user is looking at. Where the two could differ, the reference is the one that can be checked, and reading that session's screen answers the question directly.
- If the session you resolved is the one you are running in, then the agent in it is you: text sent there arrives as a message to you rather than to a new program.
