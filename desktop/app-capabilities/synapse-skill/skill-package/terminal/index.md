...
- When the user supplies a `synapse://terminals/...` link, pass it unchanged to `app_terminal_session_open` to open or focus that session instead of fetching or parsing the link. The link only works while the session still exists in the current Synapse run; if it no longer resolves, report that instead of creating a replacement session.
