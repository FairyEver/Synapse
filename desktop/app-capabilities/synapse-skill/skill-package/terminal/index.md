...
- When the user pastes a Terminal reference (lines such as `workspace_id=...`, `session_ref=...`, `session_id=...` from the Terminal sidebar or tab context menu), pass that `session_id` to `app_terminal_session_open` to open or focus the session. The reference only holds while the session still exists in the current Synapse run; if it no longer resolves, report that instead of creating a replacement session.
