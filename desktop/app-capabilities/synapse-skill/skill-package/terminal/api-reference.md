...
A Terminal reference copied from the sidebar, a tab menu, or a pane header is plain text with one `key=value` per line: `workspace_id`, `workspace_title`, `session_id`, `session_title`, and `session_ref`. `session_id` addresses the session and `workspace_id` addresses the tab that holds it (see Tabs below); the two `_title` values are what let a person — and you — recognise *which* terminal this is, since a tab split into several sessions has no other way to tell them apart in text; the checksummed `session_ref` is a local short reference and no tool accepts it. The result contains the resolved `sessionId` and nothing else: no output, no screen content, and no metadata. Sessions live only for the current Synapse run, so a reference stops resolving after its session ends or Synapse restarts; opening a stale one fails without creating or restarting a session. Never substitute a create call for a session that no longer resolves.
