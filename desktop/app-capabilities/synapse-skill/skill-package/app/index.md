...
- A task coverage assessment only covers the declared inventory. `coverage-complete` does not prove visual understanding, correct statistics, or that the inventory exhausts the user request; `semanticCorrectness: unverified` must remain unverified. Partial scope or conflicting findings remain recoverable failures. Never convert SDK success or a reported PASS into independent verification.
