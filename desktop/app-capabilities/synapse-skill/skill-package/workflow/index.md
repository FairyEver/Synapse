...
App-provided capabilities exposed as Workflow node types, including Clipboard text access, HTML generation, JSON repair, file opening, text writing, text extraction, document generation, JavaScript execution, and Node.js execution, still belong to this Workflow domain when the task is to configure or edit the node inside a Workflow. Use `app/index.md` only when directly invoking a capability that actually declares an `app_*` tool outside a Workflow definition. `clipboard_text_write`, `clipboard_text_read`, `javascript_run`, and `nodejs_run` intentionally have no direct MCP tool, System App, launcher, Automation Action, or Deep Link surface; do not invent direct Clipboard or script capability calls.
