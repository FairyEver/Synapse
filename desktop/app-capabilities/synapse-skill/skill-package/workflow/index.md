...
App-provided capabilities exposed as Workflow node types, including Clipboard text access, HTML generation, JSON repair, file opening, text writing, text extraction, document generation, JavaScript execution, and Node.js execution, still belong to this Workflow domain when the task is to configure or edit the node inside a Workflow. Use `app/index.md` only when directly invoking a capability that actually declares an `app_*` tool outside a Workflow definition. `clipboard_text_write` and `clipboard_text_read` intentionally have no direct MCP tool, System App, launcher, Automation Action, or Deep Link surface. `javascript_run` and `nodejs_run` have no direct MCP tool, System App, launcher, or Deep Link surface; their matching `builtin.javascript-run` and `builtin.nodejs-run` Automation Actions belong to the Automation domain when configuring an Automation. Do not invent direct Clipboard or script capability calls.

Only add or run `clipboard_text_write` or `clipboard_text_read` when the user currently and explicitly asks to create or run a Clipboard workflow. Never read the Clipboard to answer an ordinary question, inspect the environment, or gather context. A Workflow MCP request still executes these as Workflow nodes and does not create a separate Clipboard authorization surface.
...
A **clipboard_text_read** node requires `app.clipboard.text.read@1.0.0` and reports the whole node as `clipboard.read` high risk. A **clipboard_text_write** node requires `app.clipboard.text.write@1.0.0`, reports the whole node as `clipboard.write` high risk, and marks its persisted `text` template as sensitive without automatically redacting it from the export. Recursive export surfaces these declarations at the actual child Workflow and node.
...
- **clipboard_text_write** — Writes one explicitly configured and interpolated plain-text value to the standard system Clipboard. No provider or project needed.
- **clipboard_text_read** — Reads plain text from the standard system Clipboard. No provider or project needed.
...
Use **system_notifier_notification_trigger** when the Workflow author explicitly wants to notify the user: the node writes one message into the user's account, and Synapse delivers it to every signed-in desktop and to the user's phones.
...
- After creating, deleting, or reconnecting nodes, call `app_workflow_layout_update` before the final validation or handoff. It uses the workflow's persisted direction when `direction` is omitted. Explicit `LR` or `TB` also changes the persisted workflow layout direction together with node positions.
