...
Returns the saved task. `runMode` is `batch` or `continuous`. `concurrency` is 1-20 and means slot count. `maxRounds` is 1-500; in `batch` mode it means batch count, and in `continuous` mode it means per-slot round count. Both default to 1 when omitted. The planned worker count is `concurrency * maxRounds`; stop or cancel can make the actual started count lower. Workers run in the selected project path. `promptInjection` controls optional prompt context only; sequence/batch context, previous handoff, summary, recent-summary injection, and file-write instructions default to disabled. Previous-handoff injection forwards at most 20 records that share a 64 KiB UTF-8 budget per worker prompt; truncation and omission are marked in the injected section. `summary.recentLimit` defaults to 3, and `fileWrite.lock.enabled` defaults to true if file-write injection is enabled. `fileWrite.mode` accepts `append-only` or `update`. Synapse does not merge files or guarantee worker file writes. `fileWrite.path` must be relative to the selected project; absolute paths and parent traversal are rejected.

`agent` may optionally set `providerId`, `modelTier`, `permissionMode`, and `mainThreadPersonaId`. Use configured IDs and supported values; do not guess provider or persona IDs.
