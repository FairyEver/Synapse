...
When using `sourceDirectoryPath`, Synapse excludes `.env`, `.env.*` except the exact lowercase root filename `.env.example`, both Synapse identity files, other hidden entries, and symlinks. Case variants such as `.ENV.EXAMPLE` are runtime env-like files and are excluded without being read. Source directory imports are limited to 100 files, 200 attachment directories, depth 8, 10MB per file, and 50MB total. A root `.env.example` must also stay within the desktop installer's 1 MiB environment-file limit. If frontmatter exists in the main file, use its metadata when the user has not provided explicit fields.
