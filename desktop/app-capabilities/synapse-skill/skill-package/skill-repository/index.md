...
2. The local folder must contain root `SKILL.md` and no more than 1,000 root entries. Synapse excludes runtime `.env` files, both identity files, other hidden entries, and symlinks; excluded runtime env files are not read. Keep root `.env.example` within 1 MiB so Desktop can inspect and materialize its declarations during installation.
