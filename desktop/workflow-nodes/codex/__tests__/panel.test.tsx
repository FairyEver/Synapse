...
describe("CodexNodePanel", () => {
  it("renders unattended default controls", () => {
    ...
    expect(getByRole("button", { name: "查看模型说明" }).className).toContain("h-4 w-4")
    expect(getByRole("button", { name: "查看模型说明" }).className).toContain("text-muted-foreground")
    expect(getByRole("button", { name: "查看模型说明" }).querySelector("svg")?.getAttribute("class")).toContain("size-3")
