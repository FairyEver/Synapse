...
describe("drive URL helpers", () => {
  ...
  it("recognizes markdown drive files by extension and mime type", () => {
    ...
    expect(isDriveMarkdownItem({ name: "upload.bin", type: "file", mimeType: "text/markdown; charset=utf-8" })).toBe(true)
  ...
  it("recognizes commentable Markdown files by .md extension or MIME type", () => {
    ...
    expect(isDriveCommentableMarkdownItem({ name: "notes", type: "file", mimeType: "text/markdown; charset=utf-8" })).toBe(true)
    expect(isDriveCommentableMarkdownItem({ name: "legacy.bin", type: "file", mimeType: " TEXT/X-MARKDOWN ; charset=utf-8" })).toBe(true)
