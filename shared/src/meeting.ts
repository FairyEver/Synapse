 ...
 *
 * 取 15 分钟是给队列积压和超大文件下载留的余量，同时把这种「几十分钟的假进度 + 一句英文
 * 的失败原因」收敛成 15 分钟内的一句中文说明。
 */
 ...
 *
 * `durationMs` **只是兜底值**：服务端合并完分片之后会自己从音频的 `moov/mvhd` 里量一个
 * 时长，量到就用它。上报值实测出过两种偏差——一条 6 秒的录音报成 0 秒（`AVAudioRecorder`
 * 的 `currentTime` 读成了 0），按文件字节估出来的又把开头那段占位区算成音频（6 秒报成
 * 13 秒）。所以客户端报什么都不影响展示时长，量不到才轮到它。
 *
