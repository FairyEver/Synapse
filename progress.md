...
### 阶段 37：大目录删除回归测试
- **状态：** in_progress
- 用户要求修复正式环境大目录删除 500，提交本次代码并执行后端正式部署脚本。
- 已确认工作区存在其它未提交的 Markdown renderer 改动；本次提交必须排除这些改动，并对已被修改的 `RELEASE_NOTES_PENDING.md` 做精确分块暂存。
- 已恢复既有规划上下文，下一步读取服务端、测试和部署规则并增加 1000 文件删除回归。
- 规划文件首轮批量补丁因 `progress.md` 标题与假设不一致而未应用；已改为读取准确标题后重试。
- 新增 1000 文件目录删除回归测试；首轮 20/21 通过，新增测试按预期因事务缺少 `maxWait/timeout` 红灯。
- 为移入回收站事务增加 10 秒获取等待和 30 秒执行上限，并更新待发布说明；未改变逐条同步变更和原子回滚语义。
- 定向生命周期测试绿灯：1 个文件、21/21 通过；1000 文件回归完整确认 1001 条同步变更仍被写入。
