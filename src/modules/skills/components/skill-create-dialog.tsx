...
function SkillCreateDialog({ onOpenChange, onSubmit, open }: SkillCreateDialogProps) {
  ...
  const handleDialogOpenChange = (nextOpen: boolean) => {
    ...
    if (
      isCreateSkillPayloadDirty(form)
      && !window.confirm("放弃当前填写的 Skill 内容？")
    ) {
      return
    }
              ...
              <aside className="flex flex-col gap-3">
                ...
                <div className="rounded-lg border border-border/70 bg-muted/20 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                      <p className="text-sm font-medium text-foreground">附件摘要</p>
                      <p className="text-sm text-muted-foreground">
                        当前已选 {form.files.length} 个附件，共 {formatSkillAttachmentSize(totalAttachmentSize)}。
                      </p>
                    </div>
                  </div>
                </div>
