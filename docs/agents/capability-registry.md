...
- Agent 自动上下文维护、私有检查点、任务证据 journal 和原生 Task metadata 观察是既有执行器内部行为，不新增 capability、MCP、Workflow、Automation 或 Deep Link；运行观察继续使用同一 conversation/turn，注册数量不变。已登记范围未完成时复用既有可恢复失败投影，公开 Agent Conversation 工具仍为 11 个。
