...
Drive 分享评论通过 7 个 `app.drive.link.annotation.*` capability 暴露给 MCP：线程列表、新建线程、回复、编辑评论、删除评论、删除线程和重关联锚点。它们只接受当前 Synapse `/share/...` 下的 `.md` 文档，复用现有分享访问、评论权限和审计；不开放文档编辑、presence 或协同房间控制。
