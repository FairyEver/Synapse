...
- `PLATFORM_MEDIA_COS_*`：用户/智能体头像、系统图标、模板/分享封面、模型图标，以及浏览器文档编辑产生的平台托管图片。默认私有读写，客户端不直接接触 COS bucket/key；文档图片使用 `document-images/` 前缀，由服务端 `/object/<objectId>` 代理公开读取。生产环境必须完整配置该组变量。
