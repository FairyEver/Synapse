...
当前仅实现 SY 侧。**Portal 授权确认页仍须按下方契约实现并部署；现有普通登录页不能自动回传凭据。尚未完成真实端到端联调。** 不提供手填 token、浏览器数据库读取、Cookie 抓取或模拟成功入口。
...
真实 Portal 登录、真实 OS 浏览器协议交接、真实凭据验证及 macOS 加密联调尚待授权页交付后实施。未启动应用或浏览器，未采集用户凭据；不得将 SDK 仓库已有实测记录算作本次连接器实测。
