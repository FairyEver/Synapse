...
补充证据：[百炼直接 API 边界实测](../../reference/2026-09-13-bailian-qwen-context-probe.md)及[预算与持续对话方案](2026-09-13-bailian-context-and-continuous-conversation-plan.md)已验证近百万输入 token/6 MiB body 边界，并复现当前非文本预算的单位近似问题。执行 M03/M04 前应先核对该预算问题；本补充不替换下方历史监视基线，也不代表 M04/I06 或真实长任务验收完成。
