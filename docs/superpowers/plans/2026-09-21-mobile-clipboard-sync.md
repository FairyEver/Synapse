...
### 0.3 探针一：macOS concealed 标记（最高风险，**先做这个**）

整个隐私设计压在它身上。写一个一次性的、不提交的探针，在 Synapse 的主进程里打印 `clipboard.availableFormats()` 的完整返回值和 `clipboard.read()` 拿到的 types，然后用密码管理器（1Password / 钥匙串 / KeePassXC 任一）复制一段文本，看那个多出来的类型名到底长什么样。

- 打全量返回值，**不要只 grep `org.nspasteboard.ConcealedType`**：Qt 系工具在 macOS 上看到的是 `application/x-nspasteboard-concealed-type`，两种名字都真实存在过。
- 顺带确认普通复制（从终端选中一段、从浏览器选中一段）**不会**带上这类标记——误报会让正常内容静默丢失，比漏报更难查。

**探针不成立**：停下来告诉用户，按设计文档 6.2 的退路走（去掉过滤、如实告知），不要改成「猜密码长什么样」。

### 0.4 探针二：轮询的代价

同一个探针里量一下：空剪切板、1KB 文本、5MB 文本三种情况各调 1000 次 `readText()` 的耗时。设计定的间隔是 1 秒，要确认这个频率在真实机器上不带来可感知的开销，也要亲眼看到「大文本会很贵」这个已知代价有多大——设计接受了它，但要知道边界在哪。
