...
| 3 | 失败与边界：断网、中断、60 秒、按下即松 | 与阶段 2 一起发 |
| 4 | 真机验收与装机 | — |
...
| 60 秒上限 | 本地计到 ~58 秒主动走收尾，不等服务端断连 | 见下 |
| 按下即松 | 松手时 `phase == .idle` → 什么都不发生 | 手势 `onEnded` |

**60 秒那条是本设计里唯一没把握的地方：**

- 现在 `sendOneChunk`（`:225`）只会一直发，服务端到上限后收尾并断开，断开走 `handleSocketFailure` → `phase = .failed(.network)`，屏幕上会对还按着手指的用户写「网络已断开」——**这是误报**。
- 做法：`VoiceInputController` 新增一个只读的 `reachedLimit`，在 `sendOneChunk` 里按 `elapsed` 置位；视图观察到它就走**与松手完全相同**的收尾路径（提示「转文字中」→ 落地 → 回键盘态）。
- **先在真机上按住 60 秒测一次**，确认引擎真正断开的秒数，再定本地阈值（暂定 58）。实测结论回写 `产品设计文档.md` §5.6。

### 验证

断网可以这样造：按住说话的同时在电脑上断掉网络，或把手机切飞行模式（切飞行会走 `handleSocketFailure`）。来电图便宜：拨一个电话进来。

60 秒那条只能在真机上跑，且要有人在旁边对着麦克风持续说话 —— 这一条**排进阶段 4 的验收清单**。
...
2. 真机跑一遍设计文档 §8 的 19 条行为（第 20、21 条是单测）。
3. **装到李杨的 iPhone**：
...
- [ ] `VoiceInputController` 除新增 `reachedLimit` 外没有别的改动；
- [ ] 真机上按住 / 上滑取消 / 转文字落地 / 不回车四条亲手验过；
- [ ] 60 秒上限在真机上实测过，结论回写了设计文档 §5.6；
...
| 3 | 阶段 2：手势、气泡、预检、落地 |
| 4 | 阶段 3：失败与边界（含控制器新增的 `reachedLimit`） |
