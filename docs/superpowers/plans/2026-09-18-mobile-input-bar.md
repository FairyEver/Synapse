...
**时长这一条 2026-09-18 二次更正：真有问题，但用户侧仍然看不见。** 此前两次结论（「`max_speak_time` 只是断句、不需要新机制」）都被探针推翻了 —— `Hy-ASR-3.0-preview` 不吃 `needvad`，单连接**实测 60.90 秒**返 `4014` 后掐断（1006），攒下的文字全丢。完整实测见设计文档 §5.6。

- **`VoiceInputController` / `VoiceSession` 不再「一个字都不改」**：加了连接预热与接棒。但**对外接口与 Phase 取值集合没变** —— `start` / `retry` / `cancel` / `confirm` 四个入口、`phase` 的取值、`cancel` / `confirm` 的顺序都保持原样，视图层不需要知道轮换存在。
- **仍然不加计时、不加上限提示、不做自动收尾**，`reachedLimit` 永远作废。轮换全程 `phase` 保持 `.listening`，一次都不借 `.failed` 过渡 —— 否则视图会立刻 `settleVoice()` 把录音收掉（`TerminalScreen.swift:517-529`）。
- 接缝正确性已经先用探针量化（阶段 A：3 组 × 2 个接缝，全是硬顶切法，**6 个样本无一重复、无一丢字**），再落到两端纯值单测上。已知残余只有一个成因——定稿没按时回来（超过 2 秒上限）时只能保留冻结版，而它可能停在半截词上；详见 §5.6「已知残余」。
...
- [x] `VoiceInputController` 对外接口一个字没改 —— 改的是它内部的送包那一拍（加了连接
      预热与接棒），`start` / `retry` / `cancel` / `confirm` 与 `phase` 取值集合原样；
...
- [ ] 真机上按住说满 3 分钟验过：**数字口播**（每 30 秒一个不同数字），落地文字里每个数字
      恰好一次、顺序正确；连接不断、文字持续长、屏幕上没有任何计时或上限提示、**不出现**
      「网络已断开」。判据与接棒点位置见设计文档 §8 第 24 条；实测结论回写 §5.6。
...
6. 按住**说满 3 分钟**再松手 → 连接不断、文字持续长、屏幕上没有任何计时或上限提示、
   不出现「网络已断开」。**用数字口播**：每 30 秒报一个不同的数字（一二三…六），松手后
   数字每个恰好一次、顺序正确 —— 接棒点在 46 / 96 / 146 秒附近，正好夹在数字之间，
   丢字或重复必然打乱数字。这一条的实测结论回写设计文档 §5.6。
7. 沉默 60 秒后再按住说话 → 同样不该被掐（引擎数的是收到的采样，静音也算时长）。
