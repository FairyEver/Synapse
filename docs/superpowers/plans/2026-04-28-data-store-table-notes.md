# Data Store Table Notes Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Show table descriptions under table names in the Data Store sidebar, search by descriptions, and let users edit the table description from the schema dialog.
...
- Modify `desktop/electron/data-store/service.ts`: add `dataStoreService.updateTableDescription(table, description)`.
- Create `desktop/electron/data-store/__tests__/service.test.ts`: cover persisted table description updates.
- Modify `desktop/electron/data-store/channels.ts`: add the IPC channel constant.
- Modify `desktop/electron/data-store/ipc-handlers.ts`: register the update handler.
...
- Modify `desktop/src/modules/data-store/hooks/use-data-store.ts`: add and export the renderer wrapper.
- Modify `desktop/src/components/module-sidebar.tsx`: add optional two-line item support via `description`.
- Modify `desktop/src/modules/data-store/components/data-store-sidebar.tsx`: render descriptions and filter by name or description.
- Create `desktop/src/modules/data-store/__tests__/data-store-sidebar.test.tsx`: cover description rendering and search filtering.
- Modify `desktop/src/modules/data-store/components/table-schema-sheet.tsx`: add table description edit state and commit behavior.
- Modify `desktop/src/modules/data-store/index.tsx`: wire description updates to refresh schema and table list.
- Create `desktop/src/modules/data-store/__tests__/table-schema-sheet-layout.test.ts`: source-level guard for the table description editor surface.

## Task 1: Main-Process Table Description Persistence

**Files:**
- Create: `desktop/electron/data-store/__tests__/service.test.ts`
...
```bash
pnpm --filter @synapse/desktop exec vitest run electron/data-store/__tests__/service.test.ts
```

Expected: FAIL with a runtime error like `service.updateTableDescription is not a function`.

- [ ] **Step 3: Implement the minimal service method**

In `desktop/electron/data-store/service.ts`, add this method after `describeTable(name: string): DataStoreTableSchema`:
...
```bash
git add desktop/electron/data-store/service.ts desktop/electron/data-store/__tests__/service.test.ts
git commit -m "feat: update data store table descriptions"
```

## Task 2: IPC, Preload, Bridge, and Renderer Hook Contract

**Files:**
- Modify: `desktop/electron/data-store/channels.ts`
- Modify: `desktop/electron/data-store/ipc-handlers.ts`
- Modify: `desktop/electron/preload.ts`
- Modify: `desktop/electron/__tests__/preload.test.ts`
- Modify: `desktop/src/types/bridge.ts`
- Modify: `desktop/src/modules/data-store/hooks/use-data-store.ts`
...
Expected: FAIL with `bridge.dataStore.updateTableDescription is not a function`.

- [ ] **Step 3: Add the data-store channel**

In `desktop/electron/data-store/channels.ts`, add the channel after `describeTable`:

```ts
  updateTableDescription: "synapse:data-store:update-table-description",
```

In `desktop/electron/preload.ts`, add the same entry to `DATA_STORE_CHANNELS` after `describeTable`:
...
In `desktop/electron/data-store/ipc-handlers.ts`, add this block after the `describeTable` handler:
...
In `desktop/electron/preload.ts`, add this method after `describeTable` in the exposed `dataStore` object:
...
In `desktop/src/types/bridge.ts`, add this method after `describeTable`:
...
In `desktop/src/modules/data-store/hooks/use-data-store.ts`, add this function after `dropTable`:
...
Add `updateTableDescription` to the named export list near the existing update helpers:
...
```bash
git add desktop/electron/data-store/channels.ts desktop/electron/data-store/ipc-handlers.ts desktop/electron/preload.ts desktop/electron/__tests__/preload.test.ts desktop/src/types/bridge.ts desktop/src/modules/data-store/hooks/use-data-store.ts
...
- Modify: `desktop/src/modules/data-store/components/data-store-sidebar.tsx`
- Create: `desktop/src/modules/data-store/__tests__/data-store-sidebar.test.tsx`
...
```bash
pnpm --filter @synapse/desktop exec vitest run src/modules/data-store/__tests__/data-store-sidebar.test.tsx
```

Expected: FAIL because `filterDataStoreTables` is not exported and the sidebar does not render descriptions.
...
In `desktop/src/modules/data-store/components/data-store-sidebar.tsx`, add this helper above `DataStoreSidebar`:
...
```bash
git add desktop/src/components/module-sidebar.tsx desktop/src/modules/data-store/components/data-store-sidebar.tsx desktop/src/modules/data-store/__tests__/data-store-sidebar.test.tsx
git commit -m "feat: show data store table notes in sidebar"
```

## Task 4: Schema Dialog Table Description Editor

**Files:**
- Modify: `desktop/src/modules/data-store/components/table-schema-sheet.tsx`
- Modify: `desktop/src/modules/data-store/index.tsx`
- Create: `desktop/src/modules/data-store/__tests__/table-schema-sheet-layout.test.ts`
...
```bash
pnpm --filter @synapse/desktop exec vitest run src/modules/data-store/__tests__/table-schema-sheet-layout.test.ts
```
...
In `desktop/src/modules/data-store/components/table-schema-sheet.tsx`, change the React import to:
...
In `desktop/src/modules/data-store/components/table-schema-sheet.tsx`, inside the scrollable body and before the existing `<div className="min-h-0 overflow-auto rounded-md border">`, add:
...
- [ ] **Step 7: Wire the DataStoreModule handler**

In `desktop/src/modules/data-store/index.tsx`, add `updateTableDescription` to the hook imports:
...
```bash
git add desktop/src/modules/data-store/components/table-schema-sheet.tsx desktop/src/modules/data-store/index.tsx desktop/src/modules/data-store/__tests__/table-schema-sheet-layout.test.ts
git commit -m "feat: edit data store table notes"
```
...
```bash
pnpm --filter @synapse/desktop exec vitest run electron/data-store/__tests__/service.test.ts electron/__tests__/preload.test.ts src/modules/data-store/__tests__/data-store-sidebar.test.tsx src/modules/data-store/__tests__/table-schema-sheet-layout.test.ts
```
...
Expected: PASS. The change adds a data-store IPC handler through the existing data-store handler file and does not introduce bare `ipcMain`, direct renderer Electron access, or business-data `fs.writeFile`.
