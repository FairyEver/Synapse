...
> 已废弃（2026-09-15）：上下文预算、任务证据、输出治理与交接实现已删除；本文仅供历史追溯。参见[当前设计](../specs/2026-09-15-agent-sdk-native-context-lifecycle-design.md)。
...
| R08（部分） | Read/Bash/Grep/WebFetch 替换保持原生结构；MCP 替换移除旧 structuredContent；序列化外壳和转义计入额度。Edit/Write/NotebookEdit 的 PostToolUse 结果只是给宿主和界面的整份文件副本，模型只收到 SDK 短确认行，因此跳过截断与改写，并按 PostToolBatch 的实际交付字节记账；不能容纳完整引用或无法安全替换的其他结构继续显式停止 | 真实 SDK 出站请求验证 Read、Bash、MCP 使用替换结果，并验证 Edit/Write/NotebookEdit 只交付确认行、请求体不含整份文件；尚无无损源范围读取、交付确认及覆盖 union 账本 |
...
| R17（本次） | 覆盖缺口改为非失败提示（`task_evidence_incomplete`，保留一键继续），并让成功编辑回执、被预算截断读取的实际交付区间、单元声明 `scope` 成为合法证据；只有"声称通读但无证据"或"已登记却无任何读取/修改证据"才提示 | 真实 SDK 夹具：编辑大文件+片段读取不再提示或失败；声明未处理单元只提示不失败；覆盖判定新增单测（编辑回执、分段拼接、scope 不可变、半行不计） |
...
文件变更夹具固定另一条边界：PostToolUse 收到的 Edit/Write/NotebookEdit 结果含整份文件（如 65 KB 的 `originalFile`），PostToolBatch 传给模型的只有 35–198 字节确认行；出站请求只增长确认行，不含文件正文。治理据此跳过这类结果，不再按整份文件副本判超预算。
