...
于是「有人需要你」这个语义在本改版后有**两种呈现、两种颜色**：

- 主页内的**铃铛角标**：自绘，用 `Theme.attention`（琥珀），与会话行的「等待输入」徽章同色同义
- 底栏**主页 tab 上的未读数**：系统 `.badge`，系统红

`Theme.attention` 的契约（`DesignSystem/Theme.swift:31-42`）在自绘的部分继续成立，不受影响。
...
底栏用系统 `.badge`，颜色不改（§4.2）。App 图标角标受 §6.5.2 的开关控制。自绘的琥珀角标只出现在主页铃铛上。
...
| 通知常驻性变弱 | 底栏角标虽在，但终端页隐藏底栏，且黄色语义被系统红稀释 | 主页铃铛 + 系统推送 + 图标角标三处兜住；会话行琥珀徽章在终端内继续工作 |
