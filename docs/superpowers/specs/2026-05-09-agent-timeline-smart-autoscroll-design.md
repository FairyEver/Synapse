...
## 11. 2026-09-13 流式跟随时序补充

- 自动跟随使用即时滚动；平滑滚动仅用于用户主动点击回底，不能在每个流式增量上重新启动动画。
- 内容信号变化后，在 `useLayoutEffect` 中完成贴底，不再通过 `useEffect` 加下一帧 `requestAnimationFrame` 延迟修正，避免先显示新内容与旧滚动位置。
- 同一个 `ResizeObserver` 同时观察 viewport 和 Radix ScrollArea 内部的内容容器。图片加载、换行、过程组展开或折叠即使没有新的 timeline 信号，也在观察回调中完成贴底，不推迟到下一帧。
- 按 viewport 保存滚动位置及可滚动范围；内容收缩或视口增高将旧位置钳制到新的底部时，不视为用户上滚，也不自动恢复已暂停的跟随。即时滚动记录夹紧后的目标，平滑滚动由后续事件记录位置。
- scroll 到达时立即采样并识别用户上滚，后续帧使用事件快照；自动贴底前也检查尚未派发 scroll 的位置变化，避免抢回滚动条拖动产生的上移。用户已暂停跟随或正在恢复历史加载锚点时，尺寸变化不得抢回滚动位置。
- layout effect 与 ResizeObserver 复用同一位置判定，已贴底或误差不超过 1px 时不重复执行滚动、不刷新程序滚动保护时间。
- 回归测试必须覆盖绘制前的滚动位置、无新事件的正文尺寸变化、两种 scroll/ResizeObserver 顺序下的收缩和视口增高、scroll 派发前后的滚动条拖动，以及用户上翻后的阅读位置保持。时序测试使用可控且支持取消的帧队列，不能用微任务替代浏览器帧顺序。
