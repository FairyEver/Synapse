...
- **时机**：挂在 `flushSummary()` 里 `this.flushToolbar()` 那一行的旁边（同文件 928）。那里的注释已经
  ...
  **这份列表的刷新速度因此与工具栏完全一样，而且这里要说准（2026-09-23 修正）**：那个 tick **不是常驻
  的** —— `flushSummary` 结束时不重新武装计时器，`SUMMARY_INTERVAL_MS = 1_000` 只表示「有终端在打字时
  1 Hz」（网关自己的注释就是这么写的）。所以电脑上改了命令之后：
  - 有任何终端正在输出 → 约一秒内手机就看到；
  - 桌面完全空闲 → 要等到下一次 `sync`（手机重连）或 `attach`（在手机上打开一个终端），
    或者那台电脑上任何终端再打出一行。

  这个空窗是接受的，与工具栏接受的空窗同源，而且代价比工具栏那一份还小：手机在新建面板上拿不到新
  命令时，分组行只是没有箭头、点一下仍然直接建终端。**不为它新增监听器** —— 终端事件发射器已经到了
  Node 的监听器上限，这条规矩在这个功能上同样成立（`app-ready.ts` 里快捷输入那条线之所以能挂监听器，
  是因为它挂的是 quickInputService **自己的**发射器，不是终端那一个）。
...
`shared/src/live.ts` 的 `isLiveDesktopClientMessage` 是一个 `if` 链，**未知类型返回 `false`**；而服务端拿到
`false` 之后不是丢掉就算 —— `parseLiveDesktopMessage` 返回 `null`（`live-desktop.gateway.ts:850-864`），
消息处理的第一句就是 `socket.close(1003, "invalid_message")`（同文件 `:423-427`）。

**2026-09-23 修正**：本文档初稿把这里写成「静默丢掉，不报错、不回执」，实现期核对代码后证伪。真实的失败
模式更重：新桌面端对着老服务端发这份列表，会**被关掉那条实时连接**，两端各自重连。代价落在整条通道上，
不只是这一条消息。这条消息的发送时机是有事件才发（内容变了、手机 `sync`、手机打开终端），不是循环重试，
所以窗口内的表现是「用户正好在手机上开终端时，桌面端连接断一下再回来」，而不是持续抖动 —— 但它足以
说明升级顺序不是讲究，是硬要求。

因此必须按 **服务端 → 桌面端 → 手机端** 的顺序部署：

- 服务端先升：否则电脑一发明就被关闭连接（见上）。
- 桌面端再升：否则没有这份列表可发。
- 手机端最后：它读的就是前两端提供的东西。
...
反过来的两个组合（新手机 + 老电脑、新电脑 + 老手机）不会崩，只是**退化成今天的样子**（没箭头、点一下
直接建终端），这一点要在真机上验一遍（§8.3）。
...
5. 电脑上新增/改名/删除一条命令：**在这台电脑有任何终端正在输出**时约一秒内看到变化（新增后出现箭头，删空后箭头消失）；桌面完全空闲时，下一次手机重连或在手机上打开一个终端时同步过来（见 §3.4 的修正）。
...
- 电脑上改命令，手机面板开着看它有没有跟上（先让那台电脑上有终端在输出，再看一秒内；然后反过来验一次空闲桌面的空窗 —— 此时按 §3.4 的修正，要等到重连或开着终端才有变化）。
