# 云盘 Markdown 图片上传与转存设计（历史）

> 在线编辑器的图片扫描、归属判断与转存方案已由 `2026-09-05-document-hosted-images-design.md` 取代，不再提供对应操作；本文仅保留为历史记录。
...
相对路径图片读取与“图片转存”是两条独立链路。转存扫描继续使用 20 个来源的业务上限；预览解析最多处理当前完整 Markdown 内容中的 256 个唯一相对图片来源，不创建公共素材，也不改写 Markdown。
