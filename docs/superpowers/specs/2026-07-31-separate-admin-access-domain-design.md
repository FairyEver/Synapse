...
The browser uses a `synapse_user_session` HttpOnly opaque token backed by the existing `UserSession` service. Its lifetime follows `USER_REFRESH_TOKEN_DAYS`, and each Cookie max-age is derived from the actual server-side session expiry. The same opaque token is issued as two path-scoped cookies: `/api` authorizes ordinary user APIs, while `/drive` authorizes owner-only download and HTML render routes. A successful `/api/console/session` check refreshes the `/drive` cookie only for the remaining server-side session lifetime so existing Web sessions can open standalone Drive readers without signing in again. The browser never receives or persists access and refresh tokens. Existing desktop Bearer access tokens, refresh-token rotation and PKCE authorization remain unchanged.
