...
The browser uses a `synapse_user_session` HttpOnly opaque token backed by the existing `UserSession` service and retains the current maximum 30-day experience. The same opaque token is issued as two path-scoped cookies: `/api` authorizes ordinary user APIs, while `/drive` authorizes owner-only download and HTML render routes. A successful `/api/console/session` check refreshes the `/drive` cookie so existing Web sessions can open standalone Drive readers without signing in again. The browser never receives or persists access and refresh tokens. Existing desktop Bearer access tokens, refresh-token rotation and PKCE authorization remain unchanged.
