# Database Table Notes Design

## Context

The Database module already persists table metadata in `_meta_tables.description`. `databaseTableCreate` accepts an optional table description, `databaseTableList()` returns it as `DatabaseTableInfo.description`, and `databaseTableDescribe()` returns it as `DatabaseTableSchema.description`.

The current renderer does not use that metadata in the table sidebar. `DatabaseSidebar` shows only the raw table name and filters only by table name. The table schema dialog lets users edit column descriptions but does not expose table description editing.
...
`DatabaseModule` remains the state owner for table list, selected table, query data, and schema data.
...
- Call the new hook wrapper `databaseTableUpdate(selectedTable, description)`.
...
`DatabaseSidebar` receives the existing `DatabaseTableInfo[]` and renders `description` from each item. It keeps filtering local to the sidebar because it already owns the search query.

### Bridge and Main Process

Add a structured operation named `databaseTableUpdate` through the same layers as existing table metadata operations:

- renderer hook wrapper in `desktop/src/modules/database/hooks/use-database.ts`
- bridge type in `desktop/src/types/bridge.ts`
- preload mapping in `desktop/electron/preload.ts`
- channel in `desktop/electron/database/channels.ts`
- IPC handler in `desktop/electron/database/ipc-handlers.ts`
- service method in `desktop/electron/database/service.ts`
...
1. User opens Database module.
2. `useDatabaseTables()` loads `DatabaseTableInfo[]`, including `description`.
...
7. Renderer calls `databaseTableUpdate`.
...
- Database service test: updating a table description changes `databaseTableDescribe()` and `databaseTableList()` output.
...
- Related renderer or database tests if the repository already has a local pattern for the touched files.
...
- Keep changes scoped to Database module and the necessary bridge/service contracts.
