...
- Link autolinks keep Markdown escape sequences literally, so `https://…/shr\_a\_b` produced by an exporting tool renders the backslashes and links to a broken URL. The server renderer restores escaped punctuation inside a link whose label equals its url before the projection and the sanitized HTML are built, which keeps both text streams equal because the Renderer derives its model from that HTML. The stored Markdown source and the MDXEditor surface are not rewritten.
