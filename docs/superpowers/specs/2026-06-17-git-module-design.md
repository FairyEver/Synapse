...
- Git 仓库被设为 Agent 项目后，在对应项目对话中提供受限的常用 Git 快捷操作。
- 面向非研发用户提供短、直接、可操作的错误状态。

## 非目标

- 不合并现有 Synapse repository、resource repository、content store 或项目配置的身份与存储；Git 仓库与 Agent 项目只允许通过相同规范化绝对路径建立快捷操作关联。
- 不内置 Git runtime。
- 不做分支树视图。
- 不做可视化冲突解决。
...
当当前 Agent 项目路径与 Git 注册表中的仓库根路径精确匹配时，输入框显示 `Git` 菜单。菜单直接调用窄类型化 Git IPC，提供提交全部改动、提交并推送、拉取、推送、同步和打开当前仓库工作台；不得发送 Agent 消息、修改对话草稿或新增任意 Git 命令入口。

提交快捷操作仍使用主进程选择令牌和提交前复核。Renderer 读取完整快照，只在未截断、无冲突且仓库操作状态正常时为全部逻辑改动准备选择令牌；提交说明按 `Update 1 file via Synapse` / `Update N files via Synapse` 确定性生成并由用户确认。提交并推送严格串行，推送失败不得重复提交。认证、首次推送远端歧义、分叉、冲突和超过 10,000 项改动等状态转到 Git 应用处理。
