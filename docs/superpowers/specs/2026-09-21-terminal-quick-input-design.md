...
**快捷输入复用写入通道，不显示待执行提示条（2026-09-22 调整）。**
...
2026-09-22 调整：内容已显示在终端输入区，选中快捷输入后不再重复展示内容及「待执行 · Enter 执行」提示。成功写入时清除已有提示，仍由用户按 Enter 执行；语音输入自身的提示行为不变。
...
7. 命令条上方不显示待执行提示，仍由用户按 Enter 执行。
