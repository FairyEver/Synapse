...
- Skill 运行时 `.env` 的扫描、重新安装合并和队列更新大小上限均为 1 MiB，超限必须安全失败。在密钥关联能力中，Windows 支持扫描，但队列写入必须逐项失败并提示手动更新；不得降级为非原子写入。macOS 和 Linux 的队列写入采用最终复查后紧邻同目录原子重命名的乐观并发保护，不承诺跨进程严格 CAS。
