...
- 文本写入文件系统应用使用 `text-file-writer` app id 和 `text_file_writer` namespace；统一 capability 为 `app.text_file_writer.file.write`，MCP Tool 为 `app_text_file_writer_file_write`，Workflow 节点类型为 `text_file_writer_file_write`，首版不声明深链。App UI、MCP、Workflow 和文本提取保存适配器必须复用能力包 `main/service.ts`，统一校验绝对实际目标、`.txt/.md/.csv` 扩展名、`utf8/utf16le` 编码、默认拒绝覆盖、安全原子写入、`fs.write.outside-userdata` 权限与审计；不得设置文本长度上限、记录正文，或在入口复制校验与写入逻辑。
