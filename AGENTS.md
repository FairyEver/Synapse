...
- Skill 发布统一排除 `.env`、`.env.*`（根 `.env.example` 除外）、`.synapse.json`、`.synapse.repository.json`、其他隐藏项和符号链接；运行时 `.env` 只能按目录项名称排除，不得读取内容。`.synapse.json` 只保存资源仓库身份，云 Skill Repository 身份只保存到 `.synapse.repository.json`；旧云身份仅在云上传成功并验证新身份后迁移。
