...
- 文本写入文件系统应用使用 `text-file-writer` app id 和 `text_file_writer` namespace；统一 capability 为 `app.text_file_writer.file.write`，MCP Tool 为 `app_text_file_writer_file_write`，Workflow 节点类型为 `text_file_writer_file_write`，首版不声明深链。App UI、MCP、Workflow、文本提取保存适配器和 HTML Generator 文件能力必须复用能力包 `main/service.ts`，统一校验绝对实际目标、`.txt/.md/.csv/.html/.htm` 扩展名、默认拒绝覆盖、安全原子写入、`fs.write.outside-userdata` 权限与审计；HTML/HTM 只允许 `utf8`，其它格式继续支持 `utf8/utf16le`。Text Extractor 的保存组合层必须继续显式限制 `.txt/.md/.csv`，不得因共享 Writer 扩展而放宽；不得设置文本长度上限、记录正文，或在入口复制校验与写入逻辑。
