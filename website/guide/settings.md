...
已有仓库可修改名称和路径，也可从 Synapse AI Studio 的仓库记录中删除。删除本地配置仅移除 Synapse AI Studio 中的仓库记录，不会删除本地目录。
...
项目列表同时显示飞书连接器入口：未配置时显示“添加连接器”，已配置时显示“连接器配置”。
...
“关于”页显示 Synapse AI Studio 标识、当前版本和软件更新状态。
...
“调试”页包含日志和诊断概览。日志支持导出全部日志、复制到剪贴板、删除全部日志和打开日志目录；诊断概览可刷新并显示版本、单实例、Side-channel、Webhook、Relay、Feishu、平台、Windows 环境、PATH 和用户数据等状态。
