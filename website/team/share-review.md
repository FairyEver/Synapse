...
如果仓库位于 Git 仓库内，Synapse 会先设置本地提交身份：
...
待同步记录包含 `commitHash`、`action`、`targetId`、`title`、创建时间、重试次数和最后错误。执行待同步推送时，Synapse 会按队列尝试 `git push`；遇到 non-fast-forward、rejected 或 fetch-first 这类错误时，会先执行 `git pull --rebase`，再重试推送。
