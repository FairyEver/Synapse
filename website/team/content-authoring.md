...
创建 Rule 时，需要提供 `title`、`name`、`description`、`category`、`icon`、`iconBg`、`iconType`、`iconImage` 和 `content`。当 `iconType` 不为 `image` 时，`icon` 和 `iconBg` 也必须填写。

Rule 正文写入 `main.md`。写入时移除正文首尾空白，并保证文件以换行结尾。

`name` 是 Rule 的标识名称字段，将写入当前版本的 `snapshot.json`。若写入时为空白，快照中不会保留该字段。
