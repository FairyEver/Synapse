...
Prompts 页面复用内容模块来管理 `prompt` 类型内容。可以新建、查看、编辑和删除提示词，并查看提示词版本内容。

新建或编辑提示词时，表单包含标题、简介、分类、正文和外观字段。标题、简介和正文会按提示词表单配置进行非空校验。
...
关闭尚未提交的新建表单时，会提示当前填写内容会被清空。保存失败时，页面会显示“保存提示词失败。”作为兜底错误。
