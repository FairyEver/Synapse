# Capabilities

<!-- Sources: desktop/src/modules/apps/definitions.ts; desktop/src/modules/*; desktop/app-capabilities/document-template; desktop/synapse-capabilities/shared/registry.ts -->

本节记录桌面端公开能力和对应边界。

## 系统 App

- [系统 Apps](/advanced/system-apps)
- [Git](/advanced/git)
- [Database](/advanced/database)
- [Document Template](/advanced/document-template)
- [File Opener](/advanced/file-opener)
- [Editor Scan](/advanced/editor-scan)
- [Token Usage](/advanced/token-usage)
- [Model Price](/advanced/model-price)

## Agent 与运行能力

- [Agent](/advanced/agent)
- [Workflow](/advanced/workflow)
- [Automation](/advanced/automation)
- [Knowledge Base](/advanced/knowledge-base)

## 内容与云端资源

- [Prompts](/advanced/prompts)
- [Content MCP](/advanced/content-mcp)
- [Drive](/advanced/drive)
- [Diagnostics](/advanced/diagnostics)
