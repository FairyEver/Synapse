...
```text
状态:待确认    — 等待 agent 认领处理（巡检 Agent 创建 Issue 后的默认状态）
...
从当前 open issues 中认领 1 个同时带有 `bug` 和 `状态:待确认` label 的 issue，完成分析、修复或分类处理。
...
- 拉取最新代码。
- 确认 GitHub label 已存在：`状态:待确认`、`状态:处理中`、`状态:误判`、`状态:需要决策`。
...
- 任何关键节点都要重新读取 issue labels：
  ...
  - 提交前
  - 关闭前
- 只处理自己编号分片内、并且自己成功加上 `状态:处理中` 且移除了 `状态:待确认` 的 issue。
