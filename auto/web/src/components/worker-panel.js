...
export function WorkerPanel({ worker, lines, trimmedCount, defaultOpen = false }) {
    ...
    const Icon = cfg.icon;
    return (_jsxs("div", { className: "border border-border rounded-lg overflow-hidden", children: [_jsxs("button", { type: "button", onClick: () => setOpen(o => !o), className: "flex items-center gap-2 w-full px-3 py-2 text-sm hover:bg-muted/50 transition-colors", children: [_jsx(ChevronRight, { className: cn('h-4 w-4 transition-transform', open && 'rotate-90') }), _jsx(Icon, { className: cn('h-4 w-4', cfg.color, cfg.iconExtra) }), _jsxs("span", { className: "font-medium", children: ["Worker ", worker.id] }), _jsx("span", { className: cn('text-xs', cfg.color), children: cfg.label }), worker.durationMs > 0 && (_jsx("span", { className: "text-xs text-muted-foreground ml-auto", children: formatDuration(worker.durationMs) }))] }), open && (_jsx("div", { className: "h-64", children: lines.length > 0 ? (_jsx(Terminal, { lines: lines, trimmedCount: trimmedCount, className: "h-full" })) : (_jsx("div", { className: "h-full bg-terminal-bg flex items-center justify-center text-terminal-fg/50 text-xs font-mono", children: worker.lastMessage || '暂无输出' })) }))] }));
}
